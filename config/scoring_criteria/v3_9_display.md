...
This display release is a complete, self-contained restatement of `v3_8_display.md`, including the `Global Obtainable Peak Sales` calculation using the `1.5` US-to-global multiplier, the identity/lifecycle-as-pre-research-gate framing, and the MoA 2/3 display clarification. It spells out the neurologic, psychiatric, neuroimmune, neurodegenerative, and pain scope in Target Area Relevance display text without changing score definitions. Criteria are presented Target Relevance / MoA Validity / Data Maturity first (the three Fast Triage-shared criteria), then Competitive Landscape / Platform Attractiveness / Expansion Potential / Marketability, matching the Pipeline Table and Team Review Workspace order.
