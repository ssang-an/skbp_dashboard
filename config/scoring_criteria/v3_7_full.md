# SKBP Pipeline Finder v3.7 — Full Scoring Criteria

This release is a complete, self-contained decision reference. It retains the v3.6 Evidence Discipline, Evidence Type, canonical taxonomy, and Marketability `Global Obtainable Peak Sales` calculation with its `1.5` US-to-global multiplier, while applying two active scoring clarifications: the Full Scout PASS Target Relevance gate is `TR >= 3`, and Competitive Landscape is determined by verified differentiation and comparative evidence rather than a front-runner count alone. Identity/lifecycle remains a pre-research gate and Section 8 retains the early-stop display rule. Theme/Cluster remains classification and exploration metadata only; it is not a Target Relevance score basis.

## Canonical Score-Rule Alignment

The following inherited rules are restated here as the active v3.7 decision reference:
...
- **Platform Attractiveness 3:** first requires the 2-point quantitative comparator evidence. Then either the quantitative advantage must be reproduced across multiple conditions or platform-derived assets and independently/externally validated or used, or an officially linked platform asset must have reached First Patient Dosed. First Patient Dosed alone is insufficient.
- **Expansion Potential:** 2 requires assessed-asset early quantitative data in at least one additional indication. 3 requires multiple distinct additional indications, at least one official preclinical, IND-enabling, or clinical assessed-asset program, and asset-specific quantitative efficacy, PD, or biomarker data in that additional indication.
...
- **Competitive Landscape:** distinguish direct/high-similarity competitors from broader reference competitors. Score 3 requires both a limited direct competitive set and asset-specific, quantitative direct head-to-head evidence of a material advantage in a matched preclinical or clinical setting; no-competitor findings or competitor counts alone never establish Score 3.
...
| **Competitive Landscape** | direct/high-similarity 경쟁 대비 확인된 차별성과 현실적 진입 가능성 | 검색 또는 근거 부족으로 direct competitor와 경쟁구도 판단 불가 | 경쟁자는 확인되나 차별성은 claim 또는 이론 수준 | 적절한 comparator 대비 asset-specific 정량 차별성, 또는 기존 치료의 뚜렷한 미충족 수요를 asset-specific 근거로 해결할 현실적 진입공간 확인 | 충분한 검색 후 high-similarity 경쟁자가 제한적이고, 적절한 comparator와의 직접 head-to-head 정량 비교로 강한 우위 또는 선도 위치 확인 |
...
Broader same-disease competitor와 direct/high-similarity competitor를 구분한다. High similarity는 **주 적응증과 주 치료 가설이 실질적으로 같은 경우**, 즉 target/pathway에 대한 intervention과 그 therapeutic effector mechanism이 모두 실질적으로 겹칠 때만 적용한다. 질환명, 병리 단백질·biomarker, endpoint 또는 modality 하나만 같은 경우는 broader/reference competitor이며 high similarity로 자동 분류하지 않는다. 특히 같은 병리 단백질을 결합하더라도 primary effector pathway가 실질적으로 다르면 direct competitor 수에 자동 합산하지 않는다.

- **0점:** 검색 범위 또는 근거가 부족하여 direct competitor와 경쟁구도를 판단할 수 없다. 경쟁자를 찾지 못한 사실만으로 0점 이외의 점수를 주지 않는다.
- **1점:** direct 또는 high-similarity 경쟁자는 확인되나, assessed asset의 차별성은 회사 claim, 기전 가설 또는 비정량적 설명 수준이다.
- **2점:** 적절한 comparator 대비 assessed asset의 asset-specific 정량 차별성(예: efficacy, safety, PK/PD, delivery 또는 therapeutic window)이 확인되거나, 기존 치료의 뚜렷한 미충족 수요를 assessed asset의 target/MoA·route·safety 또는 접근성 근거로 해결할 현실적 진입공간이 확인된다. 단순히 질환에 unmet need가 크다는 사실, 시장 규모 또는 회사의 포지셔닝 claim만으로는 충분하지 않다. 직접 head-to-head 정량 우위 또는 선도 위치가 아직 확인되지 않은 경우를 포함한다.
- **3점:** 충분한 검색에서 active high-similarity 경쟁자가 제한적임을 확인하고, 적절한 high-similarity comparator와 동일하거나 비교 가능한 조건에서 수행한 assessed asset의 직접 head-to-head 정량 결과가 decision-critical endpoint(예: efficacy, safety, PK/PD, delivery 또는 therapeutic window)에서 material advantage를 입증해 선도 위치를 뒷받침한다. 전임상과 임상 모두 적용할 수 있으나, 단순한 cross-study 비교, claim, 경쟁자 부재 또는 경쟁자 수만으로는 3점을 부여하지 않는다.
