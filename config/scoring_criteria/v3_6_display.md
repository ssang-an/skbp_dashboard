# 판단근거 — SKBP Pipeline Finder v3.6

This display release is a complete, self-contained restatement of `v3_5_display.md`, including the `Global Obtainable Peak Sales` calculation using the `1.5` US-to-global multiplier. It adds the v3.6 Hard Filter's identity/lifecycle-as-pre-research-gate framing and the MoA 3 clarification that target engagement is one of three alternative asset-specific direct-validation paths, not a mandatory requirement. Criteria are presented Target Relevance / MoA Validity / Data Maturity first (the three Fast Triage-shared criteria), then Competitive Landscape / Platform Attractiveness / Expansion Potential / Marketability, matching the Pipeline Table and Team Review Workspace order.
...
- **TR:** 0 is only for an identity-verified asset with insufficient indication/relevance information; 1 is outside the broad SKBP scope; 2 is within the broad scope but outside the six priority indications; 3 is one of the six priority indications. Identity not verified is an early stop, not TR 0.
- **MoA 3:** the assessed asset directly validates the proposed MoA through one or more of target engagement, a mechanism-linked PD/biomarker, or a direct functional effect. Target engagement is not mandatory.
...
- **Competitive Landscape 3:** sufficient search is completed, high-similarity competitors are limited, and strong quantitative differentiation or a leading position is verified. Never 3 merely because no competitor was found.
...
작용기전이 얼마나 구체적으로 정의되어 있고, 이를 뒷받침하는 기능적·과학적 근거가 어느 수준인지 평가합니다. 일반 clinical efficacy만으로 3점을 부여하지 않으며 mechanism-linked PoC가 필요합니다. 3점은 target engagement, mechanism-linked PD/biomarker, 직접 functional effect 중 하나 이상으로 성립하며 target engagement가 필수는 아닙니다.
...
- **PASS:** 총점 14점 이상, TR 2점 이상, MoA 3점, Data 3점
