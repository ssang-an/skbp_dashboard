# 판단근거 — SKBP Pipeline Finder v3.8

This display release is a complete, self-contained restatement of `v3_7_display.md`, including the `Global Obtainable Peak Sales` calculation using the `1.5` US-to-global multiplier, the identity/lifecycle-as-pre-research-gate framing, and the MoA 3 clarification that target engagement is one of three alternative asset-specific direct-validation paths, not a mandatory requirement. It clarifies the display wording for MoA 2 and 3 without changing their scoring definitions. Criteria are presented Target Relevance / MoA Validity / Data Maturity first (the three Fast Triage-shared criteria), then Competitive Landscape / Platform Attractiveness / Expansion Potential / Marketability, matching the Pipeline Table and Team Review Workspace order.
