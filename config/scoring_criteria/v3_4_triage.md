# SKBP Pipeline Finder v3.4 — Fast Triage Criteria
...
| Status | Rule |
|---|---|
| **SELECT** | `identity_verified = true`, `active_asset = true`, `TR >= 2`, `MoA >= 1`, 그리고 `Data >= 2` |
| **REJECT** | Identity가 확인되고 영구 중단이 아니며 TR/MoA/Data가 모두 1점 이상이지만 SELECT 조건을 충족하지 못함. `active_asset = null`은 REJECT |
| **INSUFFICIENT** | Asset identity 미확인 또는 영구 중단으로 조기 종료했거나, identity 확인 후 TR/MoA/Data 중 하나가 0점 |

Target, MoA, indication 또는 stage만 미확인인 경우에는 해당 field를 `Unknown`으로 기록하고 scoring을 계속한다. Identity 미확인·영구 중단 조기 종료는 Markdown 점수를 `—`로 표시하며, JSON의 0점은 schema placeholder일 뿐 완료 평가 점수가 아니다.
