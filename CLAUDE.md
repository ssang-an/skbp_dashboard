...
`json/pipeline-records.json` is a flat array of analysis records and is the *only* persisted data store for investigated pipelines. Everything else (Obsidian vault, pipeline wiki vault, dashboard views) is a derived, regenerable artifact of this file. When changing record shape, update `json/drug-valuation.schema.json` (JSON Schema draft 2020-12) and `json/schema.md` (human-readable structure/rubric doc) together — `schema.md` is the authoritative explanation of section semantics (`meta`, `input`, `source_report`, `company_profile`, `rubric`, `json_summary`, `structured_table`, `hard_filter`, `scoring`, `competitive_analysis`, `validation`, `final_insight`, `obsidian`).

`json/candidate-queue.json` is a second, minimal persisted store used only by the "Step 0 진척 현황" tab: it holds pasted (asset, company) pairs that don't yet match any investigated record and therefore have no data to fit the `dashboard_hybrid_v1` schema. It is intentionally **not** part of `json/drug-valuation.schema.json`. A pending entry is not deleted from the file automatically once a matching Fast Triage/Full Scout record appears — `GET /api/candidate-queue/progress` just stops showing it as pending (folds it into that record's row instead); use `DELETE /api/candidate-queue/{queue_id}` to remove an entry outright.
...
- `POST /api/candidate-queue/import`, `GET /api/candidate-queue/progress`, and `DELETE /api/candidate-queue/{queue_id}` back the Step 0 진척 현황 tab (see "Single source of truth" above). Import dedup and progress grouping reuse `dashboard_identity_groups`/`find_matching_identity_group`, the same alias-matching logic (`company_aliases_from_text`/`asset_aliases_from_text`) that already joins Fast Triage and Full Scout records for the same asset.
...
- `POST /api/records/llm-reparse` (manual "AI 2차 파싱" button) asks OpenRouter to repair structural/missing-field paste errors strictly from the pasted Markdown, and also asks it to distill a generalized Korean caution sentence about the underlying authoring mistake when one is identifiable. `append_instruction_warning`/`load_instruction_warnings` persist those notes in `config/instruction_warnings.json` (capped/deduped per workflow mode), and `GET /api/instruction-warnings` serves them; `src/app.js`'s `copyPromptToClipboard` appends the accumulated notes to the end of the copied GPT 지침 1/지침 2 text so repeated parsing mistakes get called out before they happen again.
