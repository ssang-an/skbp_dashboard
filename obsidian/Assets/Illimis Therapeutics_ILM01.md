---
generated_from: "json/pipeline-records.json"
generated_at: "2026-08-24T19:06:21"
record_id: "Illimis Therapeutics_ILM01"
company: "Illimis Therapeutics"
country: "Republic of Korea"
asset: "ILM01"
target: "Amyloid-beta aggregates and TAM receptors"
theme: "Neuroimmune"
cluster: "Unknown"
stage: "Preclinical unspecified"
total_score: 7
max_score: 9
tags:
---

# ILM01

> Generated from `json/pipeline-records.json`. Treat JSON as the single source of truth.

## Summary

| Field | Value |
|---|---|
| Company | [[Illimis Therapeutics]] |
| Country | Republic of Korea |
| Asset | ILM01 |
| Target | Amyloid-beta aggregates and TAM receptors |
| Theme | [[Theme - Neuroimmune|Neuroimmune]] |
| Cluster | [[Cluster - Unknown|Unknown]] |
| Stage | Preclinical unspecified |
| Indication | Alzheimer's disease |
| Modality | Protein biologic |
| Hard Filter | SELECT |
| Total Score | 7 / 9 |

## One-Line Insight

SELECT: differentiated active AD lead; quantitative validation is the key gap.

## Scoring

| Criterion | Score | Evidence Type | Rubric Definition | Judgment Reason | Why Not Higher | Evidence Sources |
|---|---:|---|---|---|---|---|
| Target Relevance | 3 | triage_only | - | TR 3점: AD indication, Aβ target and TAM neuroimmune mechanism are confirmed. |  | Illimis GAIA pipeline (https://illimistx.com/kr/sub/platform%26pipeline/platform.php)<br>ILM01 ARIA-free therapy interview (https://www.koreabiomed.com/news/articleView.html?idxno=29100) |
| Competitive Landscape | - | - | - | - | - | - |
| MoA Validity | 3 | triage_only | - | MOA 3점: ILM01-specific phagocytosis, lower inflammation and cognitive improvement versus aducanumab are reported; detailed target-engagement data are absent. |  | Illimis GAIA pipeline (https://illimistx.com/kr/sub/platform%26pipeline/platform.php)<br>ILM01 ARIA-free therapy interview (https://www.koreabiomed.com/news/articleView.html?idxno=29100) |
| Platform Attractiveness | - | - | - | - | - | - |
| Expansion Potential | - | - | - | - | - | - |
| Data Maturity | 1 | triage_only | - | Data 1점: Qualitative functional evidence is public but not sufficiently quantitative. |  | ILM01 ARIA-free therapy interview (https://www.koreabiomed.com/news/articleView.html?idxno=29100) |
| Marketability | - | - | - | - | - | - |

## Detailed Scoring Rationale

### Target Relevance

| Field | Value |
|---|---|
| Score | 3 |
| Evidence Type | triage_only |
| Evidence Type Reason | TR 3점: AD indication, Aβ target and TAM neuroimmune mechanism are confirmed. |
| Rubric Definition | - |
| Judgment Reason | TR 3점: AD indication, Aβ target and TAM neuroimmune mechanism are confirmed. |
| Why Not Higher |  |
| Investigation Note |  |

#### Conflicting Or Missing Evidence

- None


#### Evidence Sources

| Source | Type | Reliability | Supports Score | Evidence Summary | URL |
|---|---|---|---|---|---|
| Illimis GAIA pipeline | official_company_pipeline | - | - | ILM01 target and mechanism. | https://illimistx.com/kr/sub/platform%26pipeline/platform.php |
| ILM01 ARIA-free therapy interview | reputable_biotech_interview | - | - | Reported lower inflammation and cognitive improvement. | https://www.koreabiomed.com/news/articleView.html?idxno=29100 |


### Competitive Landscape

| Field | Value |
|---|---|
| Score | - |
| Evidence Type | - |
| Evidence Type Reason | - |
| Rubric Definition | - |
| Judgment Reason | - |
| Why Not Higher | - |
| Investigation Note | - |

#### Conflicting Or Missing Evidence

- None


#### Evidence Sources

| Source | Type | Reliability | Supports Score | Evidence Summary | URL |
|---|---|---|---|---|---|
| None | - | - | - | - | - |


### MoA Validity

| Field | Value |
|---|---|
| Score | 3 |
| Evidence Type | triage_only |
| Evidence Type Reason | MOA 3점: ILM01-specific phagocytosis, lower inflammation and cognitive improvement versus aducanumab are reported; detailed target-engagement data are absent. |
| Rubric Definition | - |
| Judgment Reason | MOA 3점: ILM01-specific phagocytosis, lower inflammation and cognitive improvement versus aducanumab are reported; detailed target-engagement data are absent. |
| Why Not Higher |  |
| Investigation Note |  |

#### Conflicting Or Missing Evidence

- None


#### Evidence Sources

| Source | Type | Reliability | Supports Score | Evidence Summary | URL |
|---|---|---|---|---|---|
| Illimis GAIA pipeline | official_company_pipeline | - | - | ILM01 target and mechanism. | https://illimistx.com/kr/sub/platform%26pipeline/platform.php |
| ILM01 ARIA-free therapy interview | reputable_biotech_interview | - | - | Reported lower inflammation and cognitive improvement. | https://www.koreabiomed.com/news/articleView.html?idxno=29100 |


### Platform Attractiveness

| Field | Value |
|---|---|
| Score | - |
| Evidence Type | - |
| Evidence Type Reason | - |
| Rubric Definition | - |
| Judgment Reason | - |
| Why Not Higher | - |
| Investigation Note | - |

#### Conflicting Or Missing Evidence

- None


#### Evidence Sources

| Source | Type | Reliability | Supports Score | Evidence Summary | URL |
|---|---|---|---|---|---|
| None | - | - | - | - | - |


### Expansion Potential

| Field | Value |
|---|---|
| Score | - |
| Evidence Type | - |
| Evidence Type Reason | - |
| Rubric Definition | - |
| Judgment Reason | - |
| Why Not Higher | - |
| Investigation Note | - |

#### Conflicting Or Missing Evidence

- None


#### Evidence Sources

| Source | Type | Reliability | Supports Score | Evidence Summary | URL |
|---|---|---|---|---|---|
| None | - | - | - | - | - |


### Data Maturity

| Field | Value |
|---|---|
| Score | 1 |
| Evidence Type | triage_only |
| Evidence Type Reason | Data 1점: Qualitative functional evidence is public but not sufficiently quantitative. |
| Rubric Definition | - |
| Judgment Reason | Data 1점: Qualitative functional evidence is public but not sufficiently quantitative. |
| Why Not Higher |  |
| Investigation Note |  |

#### Conflicting Or Missing Evidence

- None


#### Evidence Sources

| Source | Type | Reliability | Supports Score | Evidence Summary | URL |
|---|---|---|---|---|---|
| ILM01 ARIA-free therapy interview | reputable_biotech_interview | - | - | Reported lower inflammation and cognitive improvement. | https://www.koreabiomed.com/news/articleView.html?idxno=29100 |


### Marketability

| Field | Value |
|---|---|
| Score | - |
| Evidence Type | - |
| Evidence Type Reason | - |
| Rubric Definition | - |
| Judgment Reason | - |
| Why Not Higher | - |
| Investigation Note | - |

#### Conflicting Or Missing Evidence

- None

#### Marketability A/B/C Calculation

| Step | Inputs / Assumptions | Formula | Output |
|---|---|---|---|
| A. TAP | Total patients: -; Diagnosis: -; Eligibility: -; Biomarker: -; Treatable subgroup: - | - | - |
| B. Unrisked Peak Sales | TAP: -; Annual net price: -; Peak penetration: -; Duration factor: -; Entry/share: - | - | - |
| C. Obtainable Peak Sales | Competition haircut: -; Pricing power: -; Expansion: - | - | - |


#### Evidence Sources

| Source | Type | Reliability | Supports Score | Evidence Summary | URL |
|---|---|---|---|---|---|
| None | - | - | - | - | - |


## Competitive Similarity

| Metric | Value |
|---|---:|
| Similar pipelines | 0 |
| High similarity | 0 |
| Medium similarity | 0 |
| Low similarity | 0 |



| Competitor | Company | Modality | Target or MoA | Stage | Relevance | Source |
|---|---|---|---|---|---|---|
| None | - | - | - | - | - | - |

| Company | Asset | Similarity | Matched dimensions | Shared data points |
|---|---|---:|---|---|
| None | None | - | - | - |

### Competitor Evidence Sources

| Source | Type | Reliability | Supports Score | Evidence Summary | URL |
|---|---|---|---|---|---|
| None | - | - | - | - | - |

## Key Strengths

- None

## Key Risks

- None

## Validation

### Cross-Checked Facts

- Official pipeline identifies ILM01 as Aβ-targeting lead.
- 2025 sources confirm active preclinical development and reported functional effects.

### Uncertain Points

- Claims are mainly company-sourced; IND timing is prospective.

## Source Links

- Company: [[Illimis Therapeutics]]
- Theme: [[Theme - Neuroimmune|Neuroimmune]]
- Cluster: [[Cluster - Unknown|Unknown]]
- Dashboard record id: `Illimis Therapeutics_ILM01`

## Raw JSON

```json
{
  "meta": {
    "review_type": "fast_triage",
    "schema_version": "3.2",
    "instruction_version": "3.3",
    "rubric_version": "3.3",
    "generated_at": "2026-08-06",
    "output_filename_base": "Illimis Therapeutics_ILM01",
    "storage_profile": "dashboard_hybrid_v1",
    "collaboration": {
      "comments": [
        {
          "id": "e444f4d940c9494986efe49aa36d1e0a",
          "parent_id": null,
          "author": "정주원",
          "author_user_id": "",
          "author_email": "",
          "actor_ip": "system",
          "body": "ㅇㄹㄴㅇㄹ",
          "created_at": "2026-08-22T14:26:05.566828+00:00",
          "updated_at": "2026-08-24T10:06:20.356856+00:00",
          "source": "listing_comment_post",
          "import_key": "427ab3db7b535a36ba805b61d44a12a2fb3da21c81577763fc23d18fd10d616e",
          "system_import": true
        }
      ],
      "updated_at": "2026-08-24T10:06:20.356856+00:00",
      "comment_count": 1
    },
    "edit_history": [
      {
        "id": "1c620bb9b491487dbb79e5fa46bd9b70",
        "changed_at": "2026-08-24T10:06:20.356864+00:00",
        "actor_ip": "system",
        "actor_name": "",
        "source": "cross_workflow_comment_sync",
        "field": "collaboration.comments.tab0_listing",
        "previous_value": null,
        "new_value": null
      }
    ]
  },
  "source_report": {
    "raw_markdown": "# SKBP Fast Triage Result\n\n> Version statement: This result was researched and scored with GPT instruction 1 — Fast Triage v3.2. Full Scout v3.3 has not been run.\n\n중요: 10개 후보 중 ADEL-Y04, ILM01, LNK01006은 SELECT, HBW-015와 ILM02는 REJECT, 나머지 5개는 특정 asset identity 미확인으로 UNVERIFIED입니다.\n\n| # | Asset | Company | Target/MoA | Main indication | Stage | Country | TR | MOA | Data | Triage | Why | Source |\n|---:|---|---|---|---|---|---|---:|---:|---:|---|---|---|\n| 1 | ADEL-Y04 | ADEL | ApoE4 antibody | Alzheimer's disease | Preclinical unspecified | Republic of Korea | 3 | 3 | 1 | SELECT | Active AD program; direct mouse functional evidence, limited quantitative disclosure. | S1-S3 |\n| 2 | Alzheimer's disease therapy, Beijing Youngen Biotechnology | Beijing Hotgen Biotech | Unknown | Unknown | Unknown | China | 0 | 0 | 0 | UNVERIFIED | Cannot map to Youngen APP-siRNA versus Hotgen AA001. | S1-S3 |\n| 3 | Alzheimer's Disease | Beijing Youngen Biotechnology Co Ltd | Unknown | Unknown | Unknown | China | 0 | 0 | 0 | UNVERIFIED | Indication used as asset name. | S1-S2 |\n| 4 | Drug to Inhibit Tau for Alzheimer's Disease | Hyper Corp Inc | Unknown | Alzheimer's disease | Unknown | Unknown | 0 | 0 | 0 | UNVERIFIED | No discrete Hyper Corp tau asset matched. | No verified asset source |\n| 5 | HBW-015 | Hyperway Pharmaceutical | Undisclosed pathway inhibitor | Pain | Preclinical unspecified | China | 1 | 0 | 3 | REJECT | Strong data; generic pain fit, unnamed target and active status unconfirmed. | S1-S3 |\n| 6 | ILM02 | Illimis Therapeutics | Tau/TAM GAIA fusion | Unknown (tauopathies) | Hit Discovery | Republic of Korea | 1 | 1 | 0 | REJECT | Active but broad, early and without disclosed result. | S1-S2 |\n| 7 | ILM21 | Illimis Therapeutics | Unknown | Unknown | Unknown | Republic of Korea | 0 | 0 | 0 | UNVERIFIED | No public ILM21 asset. | S1 |\n| 8 | ILM01 | Illimis Therapeutics | Aβ/TAM GAIA fusion | Alzheimer's disease | Preclinical unspecified | Republic of Korea | 3 | 3 | 1 | SELECT | Differentiated active AD lead; qualitative functional evidence. | S1-S3 |\n| 9 | Neurodegenerative Diseases Lilly | Illimis Therapeutics Inc | Unknown | Unknown | Unknown | Republic of Korea | 0 | 0 | 0 | UNVERIFIED | Collaboration descriptor, not an asset. | S1 |\n| 10 | LYNK01006 → LNK01006 | Lynk Pharmaceuticals | TYK2 allosteric inhibitor | Multiple sclerosis / neuroinflammatory disease | Phase 1 | China | 3 | 2 | 1 | SELECT | Active Phase 1 BLKR201 with CNS TYK2 rationale. | S1-S3 |\n\n## Notes\n- 입력 순서를 유지했으며, LYNK01006은 검증된 LNK01006/BLKR201로 철자 정규화했습니다.\n- Youngen/Hotgen 및 Hyper tau의 설명형 행은 다른 공개 자산으로 임의 대체하지 않았습니다.\n- 의심해볼 포인트: ADEL-Y04와 ILM01의 MOA 3은 공개된 직접 기능 결과에 기반하지만 정량 원자료가 제한적이므로 Full Scout에서 후보 동일성·통계·노출-반응을 우선 검증해야 합니다.\n- 드릴다운 질문: HBW-015의 타깃·활성·신경병증성 통증 적응증이 공식 확인되면 triage가 바뀌는가? LNK01006의 환자 적응증이 MS로 확정되지 않으면 TR 3을 유지할 수 있는가?\n- 업로드 지침 근거: :contentReference[oaicite:0]{index=0}\n\n> 이 정보가 부정확할 수 있습니다.\n",
    "parser_status": "fast_triage"
  },
  "input": {
    "company_input": "Illimis Therapeutics",
    "asset_input": "ILM01"
  },
  "json_summary": {
    "theme": "Neuroimmune",
    "cluster": "Unknown",
    "target_description": "TR 3점: AD indication, Aβ target and TAM neuroimmune mechanism are confirmed."
  },
  "structured_table": {
    "company": "Illimis Therapeutics",
    "asset_name": "ILM01",
    "target": "Amyloid-beta aggregates and TAM receptors",
    "moa": "GAIA-Aβ fusion protein for TAM-mediated amyloid phagocytosis with reduced inflammatory signaling",
    "modality_platform": "Protein biologic",
    "main_indication": "Alzheimer's disease",
    "indication": "Alzheimer's disease",
    "development_stage": "Preclinical unspecified",
    "company_country": "Republic of Korea",
    "sources": [
      {
        "source_title": "Illimis GAIA pipeline",
        "source_url": "https://illimistx.com/kr/sub/platform%26pipeline/platform.php"
      }
    ]
  },
  "hard_filter": {
    "status": "SELECT",
    "reason": "Active AD lead with asset-specific preclinical functional evidence.",
    "flags": [],
    "hard_blocker": false,
    "decision_uncertainty": false
  },
  "scoring": {
    "criteria": {
      "target_relevance": {
        "score": 3,
        "evidence_type": "triage_only",
        "evidence_type_reason": "TR 3점: AD indication, Aβ target and TAM neuroimmune mechanism are confirmed.",
        "evidence_basis": "public_source",
        "main_line_summary": "TR 3점: AD indication, Aβ target and TAM neuroimmune mechanism are confirmed.",
        "why_not_higher": "",
        "investigation_note": "",
        "uncertain_points": [],
        "source_ids": [
          "S1",
          "S2"
        ]
      },
      "moa_validity": {
        "score": 3,
        "evidence_type": "triage_only",
        "evidence_type_reason": "MOA 3점: ILM01-specific phagocytosis, lower inflammation and cognitive improvement versus aducanumab are reported; detailed target-engagement data are absent.",
        "evidence_basis": "public_source",
        "main_line_summary": "MOA 3점: ILM01-specific phagocytosis, lower inflammation and cognitive improvement versus aducanumab are reported; detailed target-engagement data are absent.",
        "why_not_higher": "",
        "investigation_note": "",
        "uncertain_points": [],
        "source_ids": [
          "S1",
          "S2"
        ]
      },
      "data_maturity": {
        "score": 1,
        "evidence_type": "triage_only",
        "evidence_type_reason": "Data 1점: Qualitative functional evidence is public but not sufficiently quantitative.",
        "evidence_basis": "public_source",
        "main_line_summary": "Data 1점: Qualitative functional evidence is public but not sufficiently quantitative.",
        "why_not_higher": "",
        "investigation_note": "",
        "uncertain_points": [],
        "source_ids": [
          "S2"
        ]
      }
    },
    "total_score": 7,
    "max_score": 9
  },
  "validation": {
    "uncertain_points": [
      "Claims are mainly company-sourced; IND timing is prospective."
    ],
    "cross_checked_facts": [
      "Official pipeline identifies ILM01 as Aβ-targeting lead.",
      "2025 sources confirm active preclinical development and reported functional effects."
    ],
    "source_registry": [
      {
        "source_id": "S1",
        "source_title": "Illimis GAIA pipeline",
        "source_url": "https://illimistx.com/kr/sub/platform%26pipeline/platform.php",
        "source_type": "official_company_pipeline",
        "evidence_summary": "ILM01 target and mechanism.",
        "verified": true
      },
      {
        "source_id": "S2",
        "source_title": "ILM01 ARIA-free therapy interview",
        "source_url": "https://www.koreabiomed.com/news/articleView.html?idxno=29100",
        "source_type": "reputable_biotech_interview",
        "evidence_summary": "Reported lower inflammation and cognitive improvement.",
        "verified": true
      },
      {
        "source_id": "S3",
        "source_title": "Illimis Series B",
        "source_url": "https://m.biospectator.com/view/news_view.php?varAtcId=25729",
        "source_type": "reputable_biotech_news",
        "evidence_summary": "Confirms active lead-program funding and preclinical development.",
        "verified": true
      }
    ]
  },
  "final_insight": {
    "one_line_summary": "SELECT: differentiated active AD lead; quantitative validation is the key gap.",
    "recommendation": "Run Full Scout",
    "most_important_diligence_question": "Does ILM01 outperform anti-Aβ antibodies on clearance, cognition and ARIA-relevant inflammation at matched exposure?"
  },
  "triage": {
    "status": "SELECT",
    "identity_verified": true,
    "active_asset": true,
    "verified_public_source_count": 2,
    "why": "Differentiated TAM-mediated amyloid clearance warrants Full Scout.",
    "missing_evidence_needed_for_full_scout": [
      "Quantitative head-to-head data",
      "ARIA-relevant vascular safety",
      "PK/brain exposure and IND-enabling evidence"
    ]
  }
}
```
