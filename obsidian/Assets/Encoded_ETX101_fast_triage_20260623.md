---
generated_from: "json/pipeline-records.json"
generated_at: "2026-08-20T15:43:42"
record_id: "Encoded_ETX101_fast_triage_20260623"
company: "Encoded Therapeutics"
country: "United States"
asset: "ETX101"
target: "SCN1A expression regulation"
theme: "E/I Balance"
cluster: "Ion Channel"
stage: "Phase 1/2"
total_score: null
max_score: 21
tags:
---

# ETX101

> Generated from `json/pipeline-records.json`. Treat JSON as the single source of truth.

## Summary

| Field | Value |
|---|---|
| Company | [[Encoded Therapeutics]] |
| Country | United States |
| Asset | ETX101 |
| Target | SCN1A expression regulation |
| Theme | [[Theme - E-I Balance|E/I Balance]] |
| Cluster | [[Cluster - Ion Channel|Ion Channel]] |
| Stage | Phase 1/2 |
| Indication | SCN1A-positive Dravet syndrome |
| Modality | AAV9 gene therapy |
| Hard Filter | SELECT |
| Total Score | None / 21 |

## One-Line Insight

High-relevance Dravet gene therapy; run Full Scout.

## Scoring

| Criterion | Score | Evidence Type | Rubric Definition | Judgment Reason | Why Not Higher | Evidence Sources |
|---|---:|---|---|---|---|---|
| Target Relevance | 3 | triage_only | - | SCN1A is directly linked to Dravet syndrome and E/I imbalance. |  | ClinicalTrials.gov ETX101 ENDEAVOR (https://clinicaltrials.gov/study/NCT05419492) |
| Competitive Landscape | - | - | - | - | - | - |
| MoA Validity | 3 | triage_only | - | AAV-mediated SCN1A expression increase is a clear root-cause strategy. |  | Encoded ETX101 Phase 1/2 interim data (https://encoded.com/press-releases/encoded-therapeutics-presents-positive-interim-efficacy-data-from-initial-dose-levels-of-phase-1-2-trials-evaluating-etx101-gene-therapy-in-dravet-syndrome/) |
| Platform Attractiveness | - | - | - | - | - | - |
| Expansion Potential | - | - | - | - | - | - |
| Data Maturity | 3 | triage_only | - | Phase 1/2 interim efficacy data and trial registry are visible. |  | Encoded ETX101 Phase 1/2 interim data (https://encoded.com/press-releases/encoded-therapeutics-presents-positive-interim-efficacy-data-from-initial-dose-levels-of-phase-1-2-trials-evaluating-etx101-gene-therapy-in-dravet-syndrome/)<br>ClinicalTrials.gov ETX101 ENDEAVOR (https://clinicaltrials.gov/study/NCT05419492) |
| Marketability | - | - | - | - | - | - |

## Detailed Scoring Rationale

### Target Relevance

| Field | Value |
|---|---|
| Score | 3 |
| Evidence Type | triage_only |
| Evidence Type Reason |  |
| Rubric Definition | - |
| Judgment Reason | SCN1A is directly linked to Dravet syndrome and E/I imbalance. |
| Why Not Higher |  |
| Investigation Note |  |

#### Conflicting Or Missing Evidence

- None


#### Evidence Sources

| Source | Type | Reliability | Supports Score | Evidence Summary | URL |
|---|---|---|---|---|---|
| ClinicalTrials.gov ETX101 ENDEAVOR | clinical_trial_registry | high | - | Phase 1/2 study of ETX101 in SCN1A-positive Dravet syndrome. | https://clinicaltrials.gov/study/NCT05419492 |


### Competitive Landscape

| Field | Value |
|---|---|
| Score | - |
| Evidence Type | - |
| Evidence Type Reason | - |
| Rubric Definition | - |
| Judgment Reason | - |
| Why Not Higher | - |
| Investigation Note | - |

#### Conflicting Or Missing Evidence

- None


#### Evidence Sources

| Source | Type | Reliability | Supports Score | Evidence Summary | URL |
|---|---|---|---|---|---|
| None | - | - | - | - | - |


### MoA Validity

| Field | Value |
|---|---|
| Score | 3 |
| Evidence Type | triage_only |
| Evidence Type Reason |  |
| Rubric Definition | - |
| Judgment Reason | AAV-mediated SCN1A expression increase is a clear root-cause strategy. |
| Why Not Higher |  |
| Investigation Note |  |

#### Conflicting Or Missing Evidence

- None


#### Evidence Sources

| Source | Type | Reliability | Supports Score | Evidence Summary | URL |
|---|---|---|---|---|---|
| Encoded ETX101 Phase 1/2 interim data | official_company | high | - | Positive interim efficacy data from Phase 1/2 trials evaluating ETX101 in Dravet syndrome. | https://encoded.com/press-releases/encoded-therapeutics-presents-positive-interim-efficacy-data-from-initial-dose-levels-of-phase-1-2-trials-evaluating-etx101-gene-therapy-in-dravet-syndrome/ |


### Platform Attractiveness

| Field | Value |
|---|---|
| Score | - |
| Evidence Type | - |
| Evidence Type Reason | - |
| Rubric Definition | - |
| Judgment Reason | - |
| Why Not Higher | - |
| Investigation Note | - |

#### Conflicting Or Missing Evidence

- None


#### Evidence Sources

| Source | Type | Reliability | Supports Score | Evidence Summary | URL |
|---|---|---|---|---|---|
| None | - | - | - | - | - |


### Expansion Potential

| Field | Value |
|---|---|
| Score | - |
| Evidence Type | - |
| Evidence Type Reason | - |
| Rubric Definition | - |
| Judgment Reason | - |
| Why Not Higher | - |
| Investigation Note | - |

#### Conflicting Or Missing Evidence

- None


#### Evidence Sources

| Source | Type | Reliability | Supports Score | Evidence Summary | URL |
|---|---|---|---|---|---|
| None | - | - | - | - | - |


### Data Maturity

| Field | Value |
|---|---|
| Score | 3 |
| Evidence Type | triage_only |
| Evidence Type Reason |  |
| Rubric Definition | - |
| Judgment Reason | Phase 1/2 interim efficacy data and trial registry are visible. |
| Why Not Higher |  |
| Investigation Note |  |

#### Conflicting Or Missing Evidence

- Long-term durability and safety need full review.


#### Evidence Sources

| Source | Type | Reliability | Supports Score | Evidence Summary | URL |
|---|---|---|---|---|---|
| Encoded ETX101 Phase 1/2 interim data | official_company | high | - | Positive interim efficacy data from Phase 1/2 trials evaluating ETX101 in Dravet syndrome. | https://encoded.com/press-releases/encoded-therapeutics-presents-positive-interim-efficacy-data-from-initial-dose-levels-of-phase-1-2-trials-evaluating-etx101-gene-therapy-in-dravet-syndrome/ |
| ClinicalTrials.gov ETX101 ENDEAVOR | clinical_trial_registry | high | - | Phase 1/2 study of ETX101 in SCN1A-positive Dravet syndrome. | https://clinicaltrials.gov/study/NCT05419492 |


### Marketability

| Field | Value |
|---|---|
| Score | - |
| Evidence Type | - |
| Evidence Type Reason | - |
| Rubric Definition | - |
| Judgment Reason | - |
| Why Not Higher | - |
| Investigation Note | - |

#### Conflicting Or Missing Evidence

- None

#### Marketability A/B/C Calculation

| Step | Inputs / Assumptions | Formula | Output |
|---|---|---|---|
| A. TAP | Total patients: -; Diagnosis: -; Eligibility: -; Biomarker: -; Treatable subgroup: - | - | - |
| B. Unrisked Peak Sales | TAP: -; Annual net price: -; Peak penetration: -; Duration factor: -; Entry/share: - | - | - |
| C. Obtainable Peak Sales | Competition haircut: -; Pricing power: -; Expansion: - | - | - |


#### Evidence Sources

| Source | Type | Reliability | Supports Score | Evidence Summary | URL |
|---|---|---|---|---|---|
| None | - | - | - | - | - |


## Competitive Similarity

| Metric | Value |
|---|---:|
| Similar pipelines | 0 |
| High similarity | 0 |
| Medium similarity | 0 |
| Low similarity | 0 |



| Competitor | Company | Modality | Target or MoA | Stage | Relevance | Source |
|---|---|---|---|---|---|---|
| None | - | - | - | - | - | - |

| Company | Asset | Similarity | Matched dimensions | Shared data points |
|---|---|---:|---|---|
| None | None | - | - | - |

### Competitor Evidence Sources

| Source | Type | Reliability | Supports Score | Evidence Summary | URL |
|---|---|---|---|---|---|
| None | - | - | - | - | - |

## Key Strengths

- None

## Key Risks

- None

## Validation

### Cross-Checked Facts

- ETX101 is an AAV9 gene therapy for SCN1A-positive Dravet syndrome.
- Phase 1/2 ENDEAVOR trial and interim efficacy data are public.

### Uncertain Points

- Fast triage did not review full patient-level dataset.

## Source Links

- Company: [[Encoded Therapeutics]]
- Theme: [[Theme - E-I Balance|E/I Balance]]
- Cluster: [[Cluster - Ion Channel|Ion Channel]]
- Dashboard record id: `Encoded_ETX101_fast_triage_20260623`

## Raw JSON

```json
{
  "meta": {
    "schema_version": "3.1",
    "rubric_version": "3.1",
    "review_type": "fast_triage",
    "generated_at": "2026-06-23",
    "output_filename_base": "Encoded_ETX101_fast_triage_20260623",
    "storage_profile": "dashboard_hybrid_v1"
  },
  "source_report": {
    "raw_markdown": "# SKBP Fast Triage Result\n\n중요: 위 10개 asset은 공개 자료상 모두 biotech/pharma pipeline asset identity가 확인되며, 빠른 triage 기준상 모두 Full Scout v3.1로 넘겨볼 만한 SELECT 후보이나, SELECT는 최종 SKBP recommendation이 아니라 심층검토 진입 신호다.\n\n| # | Asset | Company | Target/MoA | Main indication | Stage | Country | TR | MOA | Data | Triage | Why | Source |\n|---:|---|---|---|---|---|---|---:|---:|---:|---|---|---|\n| 1 | Azetukalner / XEN1101 | Xenon Pharmaceuticals | Kv7/KCNQ potassium channel opener | Epilepsy / seizure disorders | Phase 3 | Canada | 3 | 3 | 3 | SELECT | NS-041과 동일 Kv7/KCNQ 축의 가장 강한 임상 benchmark. Phase 3 FOS positive topline과 Phase 2b publication이 확인되어 Full Scout 우선순위 높음. | https://investor.xenon-pharma.com/news-releases/news-release-details/xenon-announces-positive-topline-data-phase-3-x-tole2-study<br>https://jamanetwork.com/journals/jamaneurology/fullarticle/2809938 |\n| 2 | Opakalim / BHV-7000 | Biohaven | Selective Kv7.2/Kv7.3 activator | Epilepsy / seizure disorders | Phase 2/3 | United States | 3 | 3 | 3 | SELECT | NS-041과 거의 같은 target/MoA의 same-class competitor. Biohaven 공식 pipeline상 selective Kv7.2/7.3 activator이며 pivotal epilepsy 개발 중. | https://www.biohaven.com/pipeline/innovation-engine/kv7/<br>https://www.prnewswire.com/news-releases/biohaven-reports-new-clinical-data-in-epilepsy-with-opakalim-a-selective-kv7-27-3-activator-highlighting-seizure-control-and-markedly-differentiated-tolerability-profile-302781743.html |\n| 3 | QRL-101 | QurAlis | Selective Kv7.2/Kv7.3 opener | Epilepsy / seizure disorders | Phase 1 | United States | 3 | 2 | 2 | SELECT | Kv7.2/7.3 opener로 NS-041 class 비교 가치가 있음. 현재 ALS hyperexcitability PoM/Phase 1 중심이지만 DEE Phase 2 계획이 보여 triage상 SELECT. | https://www.quralis.com/news/quralis-to-present-data-from-its-phase-1-clinical-trial-of-qrl-101-a-potential-best-in-class-selective-kv7-2-7-3-ion-channel-opener-at-american-epilepsy-society-2025-annual-meeting/ |\n| 4 | SAN2355 | Jazz Pharmaceuticals / Saniona | Selective Kv7.2/Kv7.3 activator | Epilepsy / seizure disorders | IND-enabling | Europe/UK | 3 | 2 | 1 | SELECT | Preclinical이지만 Jazz가 global exclusive license-in한 Kv7.2/7.3 epilepsy asset으로 BD benchmark 가치가 큼. Data maturity는 낮아 Full Scout에서는 자료 부족 가능성 있음. | https://investor.jazzpharma.com/news-releases/news-release-details/jazz-pharmaceuticals-enters-exclusive-licensing-agreement |\n| 5 | RAP-219 | Rapport Therapeutics | TARPγ8-specific AMPAR negative allosteric modulator | Epilepsy / seizure disorders | Phase 2 | United States | 3 | 3 | 3 | SELECT | FOS에서 novel synaptic transmission MoA이며 Phase 2a seizure/biomarker response가 확인됨. NS-041과 다른 E/I Balance approach로 비교 가치 높음. | https://investors.rapportrx.com/news-releases/news-release-details/rapport-therapeutics-presents-rap-219-focal-onset-seizure-phase/<br>https://aesnet.org/abstractslisting/efficacy-and-tolerability-of-rap-219-a-potential-first-in-class-negative-allosteric-modulator-of-%CE%B38-transmembrane-ampa-receptor-associated-protein-impact-on-rns-long-episodes-and-focal-seizures |\n| 6 | ABS-1230 | Actio Biosciences | Oral selective KCNT1 inhibitor | Epilepsy / seizure disorders | Phase 1/2 | United States | 3 | 2 | 2 | SELECT | KCNT1-related epilepsy의 genetically defined ion-channel asset. Phase 1a 및 Phase Ib/II 진입 자료가 있어 early rare-epilepsy scout 후보로 적합. | https://actiobiosciences.com/actio-biosciences-announces-first-participant-dosed-in-phase-1-clinical-trial-of-abs-1230-a-kcnt1-inhibitor-for-the-treatment-of-kcnt1-related-epilepsy/<br>https://www.clinicaltrialsarena.com/news/actio-biosciences-begins-abs-1230-trial/ |\n| 7 | Zorevunersen / STK-001 | Stoke Therapeutics / Biogen | ASO, SCN1A productive mRNA / Nav1.1 expression increase | Epilepsy / seizure disorders | Phase 3 | United States | 3 | 3 | 3 | SELECT | Dravet root-cause ASO로 Phase 3 EMPEROR 진행 및 NEJM/clinical data가 확인됨. Modality 확장성 평가용으로 Full Scout 가치 높음. | https://investor.stoketherapeutics.com/news-releases/news-release-details/stoke-therapeutics-announces-updates-timelines-completion<br>https://www.stoketherapeutics.com/patients-families/clinical-studies/ |\n| 8 | ETX101 | Encoded Therapeutics | AAV9 gene regulation therapy increasing SCN1A expression | Epilepsy / seizure disorders | Phase 1/2 | United States | 3 | 3 | 3 | SELECT | Dravet gene therapy asset로 Phase 1/2 interim efficacy가 확인됨. Zorevunersen과 disease-modifying approach 비교에 적합. | https://encoded.com/press-releases/encoded-therapeutics-presents-positive-interim-efficacy-data-from-initial-dose-levels-of-phase-1-2-trials-evaluating-etx101-gene-therapy-in-dravet-syndrome/<br>https://clinicaltrials.gov/study/NCT05419492 |\n| 9 | Bexicaserin / LP352 | Lundbeck / Longboard Pharmaceuticals | 5-HT2C receptor superagonist | Epilepsy / seizure disorders | Phase 3 | Europe/UK | 2 | 3 | 3 | SELECT | Ion channel 직접 target은 아니지만 DEE seizure에서 Phase 1b/2a 및 OLE data, Phase 3 진행이 확인되어 E/I/network modulation 관점에서 검토 가치 있음. | https://www.lundbeck.com/us/newsroom/2025/lundbeck-announces-positive-results-from-12-month-open-label-ext<br>https://pmc.ncbi.nlm.nih.gov/articles/PMC12927688/ |\n| 10 | BMB-101 | Bright Minds Biosciences | Selective 5-HT2C receptor agonist / biased agonist | Epilepsy / seizure disorders | Phase 2 | United States | 2 | 2 | 2 | SELECT | 5-HT2C seizure-modulation 계열의 smaller biotech asset. Phase 2 absence epilepsy/DEE trial identity가 확인되어 bexicaserin follow-on 비교 대상으로 적합. | https://brightmindsbio.com/bright-minds-biosciences-initiates-the-breakthrough-study-a-phase-2-trial-of-bmb-101-in-absence-epilepsy-and-developmental-epileptic-encephalopathy/<br>https://clinicaltrials.gov/study/NCT06401538 |\n\n## Notes\n- 모든 row는 quick public-source identity check 기준으로 pipeline asset identity가 확인됨.\n- Full Scout 우선순위는 Azetukalner, Opakalim, RAP-219, Zorevunersen, ETX101 순서가 가장 높음.\n- SAN2355는 SELECT이나 preclinical/IND-enabling 단계라 Full Scout에서 data maturity 부족 가능성이 큼.\n- Bexicaserin과 BMB-101은 ion-channel direct target은 아니므로 Target Relevance는 2점으로 낮게 두고, seizure clinical data와 network modulation rationale 때문에 SELECT 처리함.",
    "parser_status": "fast_triage"
  },
  "input": {
    "company_input": "Encoded Therapeutics",
    "asset_input": "ETX101"
  },
  "json_summary": {
    "theme": "E/I Balance",
    "cluster": "Ion Channel",
    "target_description": "SCN1A is directly linked to Dravet syndrome and E/I imbalance."
  },
  "structured_table": {
    "company": "Encoded Therapeutics",
    "asset_name": "ETX101",
    "target": "SCN1A expression regulation",
    "moa": "AAV9 gene regulation therapy intended to increase SCN1A expression",
    "modality_platform": "AAV9 gene therapy",
    "main_indication": "Epilepsy / seizure disorders",
    "indication": "SCN1A-positive Dravet syndrome",
    "development_stage": "Phase 1/2",
    "company_country": "United States",
    "sources": [
      {
        "source_title": "Encoded ETX101 Phase 1/2 interim data",
        "source_url": "https://encoded.com/press-releases/encoded-therapeutics-presents-positive-interim-efficacy-data-from-initial-dose-levels-of-phase-1-2-trials-evaluating-etx101-gene-therapy-in-dravet-syndrome/"
      }
    ]
  },
  "hard_filter": {
    "status": "SELECT",
    "reason": "Asset identity verified and TR/MOA/Data are all >=2; disease-modifying Dravet gene therapy with Phase 1/2 data.",
    "flags": [
      "fast_triage_only",
      "gene_therapy",
      "phase_1_2_interim_data"
    ],
    "hard_blocker": false,
    "decision_uncertainty": false
  },
  "scoring": {
    "criteria": {
      "target_relevance": {
        "score": 3,
        "evidence_type": "triage_only",
        "evidence_type_reason": "",
        "evidence_basis": "",
        "main_line_summary": "SCN1A is directly linked to Dravet syndrome and E/I imbalance.",
        "why_not_higher": "",
        "investigation_note": "",
        "uncertain_points": [],
        "source_ids": [
          "SRC_e64f34e7cfe5"
        ]
      },
      "moa_validity": {
        "score": 3,
        "evidence_type": "triage_only",
        "evidence_type_reason": "",
        "evidence_basis": "",
        "main_line_summary": "AAV-mediated SCN1A expression increase is a clear root-cause strategy.",
        "why_not_higher": "",
        "investigation_note": "",
        "uncertain_points": [],
        "source_ids": [
          "SRC_890c8794587a"
        ]
      },
      "data_maturity": {
        "score": 3,
        "evidence_type": "triage_only",
        "evidence_type_reason": "",
        "evidence_basis": "",
        "main_line_summary": "Phase 1/2 interim efficacy data and trial registry are visible.",
        "why_not_higher": "",
        "investigation_note": "",
        "uncertain_points": [
          "Long-term durability and safety need full review."
        ],
        "source_ids": [
          "SRC_890c8794587a",
          "SRC_e64f34e7cfe5"
        ]
      }
    },
    "total_score": null,
    "max_score": 21
  },
  "validation": {
    "uncertain_points": [
      "Fast triage did not review full patient-level dataset."
    ],
    "cross_checked_facts": [
      "ETX101 is an AAV9 gene therapy for SCN1A-positive Dravet syndrome.",
      "Phase 1/2 ENDEAVOR trial and interim efficacy data are public."
    ],
    "source_registry": [
      {
        "source_id": "SRC_890c8794587a",
        "source_title": "Encoded ETX101 Phase 1/2 interim data",
        "source_url": "https://encoded.com/press-releases/encoded-therapeutics-presents-positive-interim-efficacy-data-from-initial-dose-levels-of-phase-1-2-trials-evaluating-etx101-gene-therapy-in-dravet-syndrome/",
        "source_type": "official_company",
        "reliability": "high",
        "evidence_summary": "Positive interim efficacy data from Phase 1/2 trials evaluating ETX101 in Dravet syndrome."
      },
      {
        "source_id": "SRC_e64f34e7cfe5",
        "source_title": "ClinicalTrials.gov ETX101 ENDEAVOR",
        "source_url": "https://clinicaltrials.gov/study/NCT05419492",
        "source_type": "clinical_trial_registry",
        "reliability": "high",
        "evidence_summary": "Phase 1/2 study of ETX101 in SCN1A-positive Dravet syndrome."
      }
    ]
  },
  "final_insight": {
    "one_line_summary": "High-relevance Dravet gene therapy; run Full Scout.",
    "recommendation": "Run Full Scout",
    "most_important_diligence_question": "Can ETX101 demonstrate durable benefit with acceptable AAV safety in very young Dravet patients?"
  },
  "triage": {
    "status": "SELECT",
    "identity_verified": true,
    "active_asset": null,
    "verified_public_source_count": 2,
    "why": "SCN1A/Dravet biology is highly relevant and Phase 1/2 interim clinical data are visible.",
    "missing_evidence_needed_for_full_scout": [
      "Durability of seizure reduction",
      "Age-window and dosing constraints",
      "AAV safety and immunogenicity",
      "Comparison to zorevunersen"
    ]
  }
}
```
