---
generated_from: "json/pipeline-records.json"
generated_at: "2026-08-07T23:02:38"
record_id: "Beijing Hotgen Biotech_Alzheimer's disease therapy, Beijing Youngen Biotechnology"
company: "Beijing Hotgen Biotech"
country: "China"
asset: "Alzheimer's disease therapy, Beijing Youngen Biotechnology"
target: "Unknown"
theme: "Unknown"
cluster: "Unknown"
stage: "Unknown"
total_score: 0
max_score: 9
tags:
---

# Alzheimer's disease therapy, Beijing Youngen Biotechnology

> Generated from `json/pipeline-records.json`. Treat JSON as the single source of truth.

## Summary

| Field | Value |
|---|---|
| Company | [[Beijing Hotgen Biotech]] |
| Country | China |
| Asset | Alzheimer's disease therapy, Beijing Youngen Biotechnology |
| Target | Unknown |
| Theme | [[Theme - Unknown|Unknown]] |
| Cluster | [[Cluster - Unknown|Unknown]] |
| Stage | Unknown |
| Indication | Unknown |
| Modality | Unknown |
| Hard Filter | UNVERIFIED |
| Total Score | 0 / 9 |

## One-Line Insight

UNVERIFIED: generic description cannot be mapped to one asset.

## Scoring

| Criterion | Score | Evidence Type | Rubric Definition | Judgment Reason | Why Not Higher | Evidence Sources |
|---|---:|---|---|---|---|---|
| Target Relevance | 0 | triage_only | - | TR 0점: No specific asset identity is verified. |  | - |
| Competitive Landscape | - | - | - | - | - | - |
| MoA Validity | 0 | triage_only | - | MOA 0점: Mapping to APP-siRNA or AA001 would be an unsupported substitution. |  | - |
| Platform Attractiveness | - | - | - | - | - | - |
| Expansion Potential | - | - | - | - | - | - |
| Data Maturity | 0 | triage_only | - | Data 0점: No result is tied to this exact label. |  | - |
| Marketability | - | - | - | - | - | - |

## Detailed Scoring Rationale

### Target Relevance

| Field | Value |
|---|---|
| Score | 0 |
| Evidence Type | triage_only |
| Evidence Type Reason | TR 0점: No specific asset identity is verified. |
| Rubric Definition | - |
| Judgment Reason | TR 0점: No specific asset identity is verified. |
| Why Not Higher |  |
| Investigation Note |  |

#### Conflicting Or Missing Evidence

- None


#### Evidence Sources

| Source | Type | Reliability | Supports Score | Evidence Summary | URL |
|---|---|---|---|---|---|
| None | - | - | - | - | - |


### Competitive Landscape

| Field | Value |
|---|---|
| Score | - |
| Evidence Type | - |
| Evidence Type Reason | - |
| Rubric Definition | - |
| Judgment Reason | - |
| Why Not Higher | - |
| Investigation Note | - |

#### Conflicting Or Missing Evidence

- None


#### Evidence Sources

| Source | Type | Reliability | Supports Score | Evidence Summary | URL |
|---|---|---|---|---|---|
| None | - | - | - | - | - |


### MoA Validity

| Field | Value |
|---|---|
| Score | 0 |
| Evidence Type | triage_only |
| Evidence Type Reason | MOA 0점: Mapping to APP-siRNA or AA001 would be an unsupported substitution. |
| Rubric Definition | - |
| Judgment Reason | MOA 0점: Mapping to APP-siRNA or AA001 would be an unsupported substitution. |
| Why Not Higher |  |
| Investigation Note |  |

#### Conflicting Or Missing Evidence

- None


#### Evidence Sources

| Source | Type | Reliability | Supports Score | Evidence Summary | URL |
|---|---|---|---|---|---|
| None | - | - | - | - | - |


### Platform Attractiveness

| Field | Value |
|---|---|
| Score | - |
| Evidence Type | - |
| Evidence Type Reason | - |
| Rubric Definition | - |
| Judgment Reason | - |
| Why Not Higher | - |
| Investigation Note | - |

#### Conflicting Or Missing Evidence

- None


#### Evidence Sources

| Source | Type | Reliability | Supports Score | Evidence Summary | URL |
|---|---|---|---|---|---|
| None | - | - | - | - | - |


### Expansion Potential

| Field | Value |
|---|---|
| Score | - |
| Evidence Type | - |
| Evidence Type Reason | - |
| Rubric Definition | - |
| Judgment Reason | - |
| Why Not Higher | - |
| Investigation Note | - |

#### Conflicting Or Missing Evidence

- None


#### Evidence Sources

| Source | Type | Reliability | Supports Score | Evidence Summary | URL |
|---|---|---|---|---|---|
| None | - | - | - | - | - |


### Data Maturity

| Field | Value |
|---|---|
| Score | 0 |
| Evidence Type | triage_only |
| Evidence Type Reason | Data 0점: No result is tied to this exact label. |
| Rubric Definition | - |
| Judgment Reason | Data 0점: No result is tied to this exact label. |
| Why Not Higher |  |
| Investigation Note |  |

#### Conflicting Or Missing Evidence

- None


#### Evidence Sources

| Source | Type | Reliability | Supports Score | Evidence Summary | URL |
|---|---|---|---|---|---|
| None | - | - | - | - | - |


### Marketability

| Field | Value |
|---|---|
| Score | - |
| Evidence Type | - |
| Evidence Type Reason | - |
| Rubric Definition | - |
| Judgment Reason | - |
| Why Not Higher | - |
| Investigation Note | - |

#### Conflicting Or Missing Evidence

- None

#### Marketability A/B/C Calculation

| Step | Inputs / Assumptions | Formula | Output |
|---|---|---|---|
| A. TAP | Total patients: -; Diagnosis: -; Eligibility: -; Biomarker: -; Treatable subgroup: - | - | - |
| B. Unrisked Peak Sales | TAP: -; Annual net price: -; Peak penetration: -; Duration factor: -; Entry/share: - | - | - |
| C. Obtainable Peak Sales | Competition haircut: -; Pricing power: -; Expansion: - | - | - |


#### Evidence Sources

| Source | Type | Reliability | Supports Score | Evidence Summary | URL |
|---|---|---|---|---|---|
| None | - | - | - | - | - |


## Competitive Similarity

| Metric | Value |
|---|---:|
| Similar pipelines | 0 |
| High similarity | 0 |
| Medium similarity | 0 |
| Low similarity | 0 |



| Competitor | Company | Modality | Target or MoA | Stage | Relevance | Source |
|---|---|---|---|---|---|---|
| None | - | - | - | - | - | - |

| Company | Asset | Similarity | Matched dimensions | Shared data points |
|---|---|---:|---|---|
| None | None | - | - | - |

### Competitor Evidence Sources

| Source | Type | Reliability | Supports Score | Evidence Summary | URL |
|---|---|---|---|---|---|
| None | - | - | - | - | - |

## Key Strengths

- None

## Key Risks

- None

## Validation

### Cross-Checked Facts

- Youngen is a Hotgen subsidiary.
- Youngen disclosed an APP-siRNA AD patent; Hotgen separately develops AA001.

### Uncertain Points

- Likely a database-generated program description.

## Source Links

- Company: [[Beijing Hotgen Biotech]]
- Theme: [[Theme - Unknown|Unknown]]
- Cluster: [[Cluster - Unknown|Unknown]]
- Dashboard record id: `Beijing Hotgen Biotech_Alzheimer's disease therapy, Beijing Youngen Biotechnology`

## Raw JSON

```json
{
  "meta": {
    "review_type": "fast_triage",
    "schema_version": "3.2",
    "instruction_version": "3.3",
    "rubric_version": "3.3",
    "generated_at": "2026-08-06",
    "output_filename_base": "Beijing Hotgen Biotech_Alzheimer's disease therapy, Beijing Youngen Biotechnology",
    "storage_profile": "dashboard_hybrid_v1"
  },
  "source_report": {
    "raw_markdown": "# SKBP Fast Triage Result\n\n> Version statement: This result was researched and scored with GPT instruction 1 — Fast Triage v3.2. Full Scout v3.3 has not been run.\n\n중요: 10개 후보 중 ADEL-Y04, ILM01, LNK01006은 SELECT, HBW-015와 ILM02는 REJECT, 나머지 5개는 특정 asset identity 미확인으로 UNVERIFIED입니다.\n\n| # | Asset | Company | Target/MoA | Main indication | Stage | Country | TR | MOA | Data | Triage | Why | Source |\n|---:|---|---|---|---|---|---|---:|---:|---:|---|---|---|\n| 1 | ADEL-Y04 | ADEL | ApoE4 antibody | Alzheimer's disease | Preclinical unspecified | Republic of Korea | 3 | 3 | 1 | SELECT | Active AD program; direct mouse functional evidence, limited quantitative disclosure. | S1-S3 |\n| 2 | Alzheimer's disease therapy, Beijing Youngen Biotechnology | Beijing Hotgen Biotech | Unknown | Unknown | Unknown | China | 0 | 0 | 0 | UNVERIFIED | Cannot map to Youngen APP-siRNA versus Hotgen AA001. | S1-S3 |\n| 3 | Alzheimer's Disease | Beijing Youngen Biotechnology Co Ltd | Unknown | Unknown | Unknown | China | 0 | 0 | 0 | UNVERIFIED | Indication used as asset name. | S1-S2 |\n| 4 | Drug to Inhibit Tau for Alzheimer's Disease | Hyper Corp Inc | Unknown | Alzheimer's disease | Unknown | Unknown | 0 | 0 | 0 | UNVERIFIED | No discrete Hyper Corp tau asset matched. | No verified asset source |\n| 5 | HBW-015 | Hyperway Pharmaceutical | Undisclosed pathway inhibitor | Pain | Preclinical unspecified | China | 1 | 0 | 3 | REJECT | Strong data; generic pain fit, unnamed target and active status unconfirmed. | S1-S3 |\n| 6 | ILM02 | Illimis Therapeutics | Tau/TAM GAIA fusion | Unknown (tauopathies) | Hit Discovery | Republic of Korea | 1 | 1 | 0 | REJECT | Active but broad, early and without disclosed result. | S1-S2 |\n| 7 | ILM21 | Illimis Therapeutics | Unknown | Unknown | Unknown | Republic of Korea | 0 | 0 | 0 | UNVERIFIED | No public ILM21 asset. | S1 |\n| 8 | ILM01 | Illimis Therapeutics | Aβ/TAM GAIA fusion | Alzheimer's disease | Preclinical unspecified | Republic of Korea | 3 | 3 | 1 | SELECT | Differentiated active AD lead; qualitative functional evidence. | S1-S3 |\n| 9 | Neurodegenerative Diseases Lilly | Illimis Therapeutics Inc | Unknown | Unknown | Unknown | Republic of Korea | 0 | 0 | 0 | UNVERIFIED | Collaboration descriptor, not an asset. | S1 |\n| 10 | LYNK01006 → LNK01006 | Lynk Pharmaceuticals | TYK2 allosteric inhibitor | Multiple sclerosis / neuroinflammatory disease | Phase 1 | China | 3 | 2 | 1 | SELECT | Active Phase 1 BLKR201 with CNS TYK2 rationale. | S1-S3 |\n\n## Notes\n- 입력 순서를 유지했으며, LYNK01006은 검증된 LNK01006/BLKR201로 철자 정규화했습니다.\n- Youngen/Hotgen 및 Hyper tau의 설명형 행은 다른 공개 자산으로 임의 대체하지 않았습니다.\n- 의심해볼 포인트: ADEL-Y04와 ILM01의 MOA 3은 공개된 직접 기능 결과에 기반하지만 정량 원자료가 제한적이므로 Full Scout에서 후보 동일성·통계·노출-반응을 우선 검증해야 합니다.\n- 드릴다운 질문: HBW-015의 타깃·활성·신경병증성 통증 적응증이 공식 확인되면 triage가 바뀌는가? LNK01006의 환자 적응증이 MS로 확정되지 않으면 TR 3을 유지할 수 있는가?\n- 업로드 지침 근거: :contentReference[oaicite:0]{index=0}",
    "parser_status": "fast_triage"
  },
  "input": {
    "company_input": "Beijing Hotgen Biotech",
    "asset_input": "Alzheimer's disease therapy, Beijing Youngen Biotechnology"
  },
  "json_summary": {
    "theme": "Unknown",
    "cluster": "Unknown",
    "target_description": "TR 0점: No specific asset identity is verified."
  },
  "structured_table": {
    "company": "Beijing Hotgen Biotech",
    "asset_name": "Alzheimer's disease therapy, Beijing Youngen Biotechnology",
    "target": "Unknown",
    "moa": "Unknown",
    "modality_platform": "Unknown",
    "main_indication": "Unknown",
    "indication": "Unknown",
    "development_stage": "Unknown",
    "company_country": "China",
    "sources": [
      {
        "source_title": "Youngen overview",
        "source_url": "https://www.youngenbiomed.com/"
      }
    ]
  },
  "hard_filter": {
    "status": "UNVERIFIED",
    "reason": "Generic label cannot be assigned to one candidate.",
    "flags": [
      "identity_not_verified",
      "possible_parent_subsidiary_mismatch"
    ],
    "hard_blocker": false,
    "decision_uncertainty": false
  },
  "scoring": {
    "criteria": {
      "target_relevance": {
        "score": 0,
        "evidence_type": "triage_only",
        "evidence_type_reason": "TR 0점: No specific asset identity is verified.",
        "evidence_basis": "no_supporting_basis",
        "main_line_summary": "TR 0점: No specific asset identity is verified.",
        "why_not_higher": "",
        "investigation_note": "",
        "uncertain_points": [],
        "source_ids": []
      },
      "moa_validity": {
        "score": 0,
        "evidence_type": "triage_only",
        "evidence_type_reason": "MOA 0점: Mapping to APP-siRNA or AA001 would be an unsupported substitution.",
        "evidence_basis": "no_supporting_basis",
        "main_line_summary": "MOA 0점: Mapping to APP-siRNA or AA001 would be an unsupported substitution.",
        "why_not_higher": "",
        "investigation_note": "",
        "uncertain_points": [],
        "source_ids": []
      },
      "data_maturity": {
        "score": 0,
        "evidence_type": "triage_only",
        "evidence_type_reason": "Data 0점: No result is tied to this exact label.",
        "evidence_basis": "no_supporting_basis",
        "main_line_summary": "Data 0점: No result is tied to this exact label.",
        "why_not_higher": "",
        "investigation_note": "",
        "uncertain_points": [],
        "source_ids": []
      }
    },
    "total_score": 0,
    "max_score": 9
  },
  "validation": {
    "uncertain_points": [
      "Likely a database-generated program description."
    ],
    "cross_checked_facts": [
      "Youngen is a Hotgen subsidiary.",
      "Youngen disclosed an APP-siRNA AD patent; Hotgen separately develops AA001."
    ],
    "source_registry": [
      {
        "source_id": "S1",
        "source_title": "Youngen overview",
        "source_url": "https://www.youngenbiomed.com/",
        "source_type": "official_company_page",
        "evidence_summary": "Confirms Youngen-Hotgen relationship.",
        "verified": true
      },
      {
        "source_id": "S2",
        "source_title": "Youngen APP-siRNA AD patent",
        "source_url": "https://www.youngenbiomed.com/youngen-biotechnology-patent-for-small-nucleic-acid-therapeutic-drugs-for-alzheimers-disease-granted.html",
        "source_type": "official_company_news",
        "evidence_summary": "Relevant research but no asset code/stage.",
        "verified": true
      },
      {
        "source_id": "S3",
        "source_title": "Hotgen AA001 report",
        "source_url": "https://flcube.com/?p=58620",
        "source_type": "industry_news",
        "evidence_summary": "Separate anti-Aβ asset, showing ambiguity.",
        "verified": true
      }
    ]
  },
  "final_insight": {
    "one_line_summary": "UNVERIFIED: generic description cannot be mapped to one asset.",
    "recommendation": "Verify asset identity",
    "most_important_diligence_question": "Is this Youngen's APP-siRNA program, Hotgen's AA001, or another coded candidate?"
  },
  "triage": {
    "status": "UNVERIFIED",
    "identity_verified": false,
    "active_asset": null,
    "verified_public_source_count": 0,
    "why": "Youngen has APP-siRNA work and Hotgen has AA001, but this row does not identify either asset.",
    "missing_evidence_needed_for_full_scout": [
      "Exact asset code",
      "Owner, target, modality and stage"
    ]
  }
}
```
