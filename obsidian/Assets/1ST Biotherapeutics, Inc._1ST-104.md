---
generated_from: "json/pipeline-records.json"
generated_at: "2026-08-21T09:06:05"
record_id: "1ST Biotherapeutics, Inc._1ST-104"
company: "1ST Biotherapeutics, Inc."
country: "Republic of Korea"
asset: "1ST-104"
target: "LRRK2 / c-Abl"
theme: "Neuroimmune"
cluster: "Cytokine 신경조절"
stage: "Lead Optimization"
total_score: 12
max_score: 21
tags:
---

# 1ST-104

> Generated from `json/pipeline-records.json`. Treat JSON as the single source of truth.

## Summary

| Field | Value |
|---|---|
| Company | [[1ST Biotherapeutics, Inc]] |
| Country | Republic of Korea |
| Asset | 1ST-104 |
| Target | LRRK2 / c-Abl |
| Theme | [[Theme - Neuroimmune|Neuroimmune]] |
| Cluster | [[Cluster - Cytokine 신경조절|Cytokine 신경조절]] |
| Stage | Lead Optimization |
| Indication | Parkinson's disease |
| Modality | Small molecule |
| Hard Filter | REVIEW |
| Total Score | 12 / 21 |

## One-Line Insight

1ST-104 combines strong PD target relevance and broad lead-optimization data with a differentiated Type II-like profile, but candidate, efficacy, safety and commercial de-risking remain incomplete in an increasingly challenging LRRK2 field.

## Scoring

| Criterion | Score | Evidence Type | Rubric Definition | Judgment Reason | Why Not Higher | Evidence Sources |
|---|---:|---|---|---|---|---|
| Target Relevance | 3 | E3_asset_specific_preclinical_or_technical_evidence | - | Strong PD fit with genetically validated LRRK2 biology and asset-specific dual-target activity. | Score 3 is the maximum; clinical proof of benefit for the dual-target strategy is still absent. | 1ST Biotherapeutics R&D Pipeline (https://www.1stbio.com/rd-pipeline/)<br>[104] Current Progress and Key Findings_NCDP.pdf<br>Mutations in LRRK2 cause autosomal-dominant parkinsonism with pleomorphic pathology (https://pubmed.ncbi.nlm.nih.gov/15541309/) |
| Competitive Landscape | 2 | E3_asset_specific_preclinical_or_technical_evidence | - | Quantitative differentiation exists, but several clinical LRRK2 competitors are ahead and recent efficacy failures increase risk. | No clinical or independently replicated head-to-head differentiation; multiple high-similarity programs are ahead. | [104] Current Progress and Key Findings_NCDP.pdf<br>NEU-411 Phase 2 NEULARK - NCT06680830 (https://clinicaltrials.gov/study/NCT06680830)<br>BIIB122 Phase 2b LUMA Update (https://investors.biogen.com/news-releases/news-release-details/biogen-and-denali-therapeutics-provide-update-phase-2b-luma) |
| MoA Validity | 3 | E3_asset_specific_preclinical_or_technical_evidence | - | Strong direct preclinical validation of intended dual-target pharmacology. | Score 3 is the maximum; LRRK2 Type II binding itself has not yet been structurally confirmed for the assessed compounds. | [104] Current Progress and Key Findings_NCDP.pdf<br>c-Abl phosphorylates alpha-synuclein and regulates its degradation (https://pubmed.ncbi.nlm.nih.gov/24412932/)<br>Phos-tag analysis of Rab10 phosphorylation by LRRK2 (https://pubmed.ncbi.nlm.nih.gov/27474410/) |
| Platform Attractiveness | 1 | E1_company_claim_or_scientific_rationale_only | - | Reusable AI/medchem infrastructure is credible, but differentiated technical performance is not quantitatively demonstrated. | No platform-vs-comparator quantitative result on hit rate, cycle time, quality or cost. | 1ST Biotherapeutics R&D Pipeline (https://www.1stbio.com/rd-pipeline/)<br>[104] Current Progress and Key Findings_NCDP.pdf |
| Expansion Potential | 0 | E0_not_found_or_not_assessable | - | No confirmed asset-specific indication expansion beyond Parkinson's disease. | No additional indication with explicit 1ST-104 biological rationale or program status was verified. | 1ST Biotherapeutics R&D Pipeline (https://www.1stbio.com/rd-pipeline/)<br>[104] Current Progress and Key Findings_NCDP.pdf |
| Data Maturity | 3 | E3_asset_specific_preclinical_or_technical_evidence | - | Broad and quantitative lead-optimization package across several development questions. | Score 3 is the maximum; candidate nomination, completed in vivo efficacy and development-grade toxicology remain outstanding. | [104] Current Progress and Key Findings_NCDP.pdf |
| Marketability | 0 | E0_not_found_or_not_assessable | - | Commercial opportunity may be large, but asset-specific obtainable peak sales cannot yet be reliably quantified. | No reliable calculation or asset-specific external forecast supports a sales threshold. | BIIB122 Phase 2b LUMA Update (https://investors.biogen.com/news-releases/news-release-details/biogen-and-denali-therapeutics-provide-update-phase-2b-luma)<br>Parkinson's Foundation Prevalence and Incidence (https://www.parkinson.org/understanding-parkinsons/statistics/prevalence-incidence)<br>BIIB122 Phase 2a BEACON - NCT06602193 (https://clinicaltrials.gov/study/NCT06602193) |

## Detailed Scoring Rationale

### Target Relevance

| Field | Value |
|---|---|
| Score | 3 |
| Evidence Type | E3_asset_specific_preclinical_or_technical_evidence |
| Evidence Type Reason | 1ST-104 is confirmed as an LRRK2/c-Abl Parkinson's program with direct asset-specific target pharmacology, supported by independent PD target biology. |
| Rubric Definition | - |
| Judgment Reason | Strong PD fit with genetically validated LRRK2 biology and asset-specific dual-target activity. |
| Why Not Higher | Score 3 is the maximum; clinical proof of benefit for the dual-target strategy is still absent. |
| Investigation Note | LRRK2 provides the strongest disease-biology anchor; c-Abl adds mechanistic rationale but weaker clinical validation. |

#### Conflicting Or Missing Evidence

- Incremental clinical contribution of c-Abl inhibition is unproven.


#### Evidence Sources

| Source | Type | Reliability | Supports Score | Evidence Summary | URL |
|---|---|---|---|---|---|
| 1ST Biotherapeutics R&D Pipeline | official_company | - | - | - | https://www.1stbio.com/rd-pipeline/ |
| [104] Current Progress and Key Findings_NCDP.pdf | user_uploaded_file | - | - | - | - |
| Mutations in LRRK2 cause autosomal-dominant parkinsonism with pleomorphic pathology | peer_reviewed | - | - | - | https://pubmed.ncbi.nlm.nih.gov/15541309/ |
| c-Abl phosphorylates alpha-synuclein and regulates its degradation | peer_reviewed | - | - | - | https://pubmed.ncbi.nlm.nih.gov/24412932/ |


### Competitive Landscape

| Field | Value |
|---|---|
| Score | 2 |
| Evidence Type | E3_asset_specific_preclinical_or_technical_evidence |
| Evidence Type Reason | Internal quantitative pRab10/pS935 comparison supports differentiation versus a Type I comparator, while multiple clinical LRRK2 programs define a crowded field. |
| Rubric Definition | - |
| Judgment Reason | Quantitative differentiation exists, but several clinical LRRK2 competitors are ahead and recent efficacy failures increase risk. |
| Why Not Higher | No clinical or independently replicated head-to-head differentiation; multiple high-similarity programs are ahead. |
| Investigation Note | LUMA and vodobatinib failures increase the importance of patient selection and proof that dual/Type II pharmacology changes efficacy. |

#### Conflicting Or Missing Evidence

- Clinical relevance of pS935 preservation is not established.
- Exact entry timing versus Phase 2 competitors depends on candidate nomination and IND progression.


#### Evidence Sources

| Source | Type | Reliability | Supports Score | Evidence Summary | URL |
|---|---|---|---|---|---|
| [104] Current Progress and Key Findings_NCDP.pdf | user_uploaded_file | - | - | - | - |
| NEU-411 Phase 2 NEULARK - NCT06680830 | clinical_registry | - | - | - | https://clinicaltrials.gov/study/NCT06680830 |
| BIIB122 Phase 2b LUMA Update | company_press_release | - | - | - | https://investors.biogen.com/news-releases/news-release-details/biogen-and-denali-therapeutics-provide-update-phase-2b-luma |
| Brenig Therapeutics BT-267 Pipeline | official_company | - | - | - | https://www.brenigther.com/ |
| Arvinas ARV-102 Phase 1 Parkinson's Disease Data | company_press_release | - | - | - | https://ir.arvinas.com/news-releases/news-release-details/arvinas-announces-positive-phase-1-data-arv-102-showing-greater/ |
| BIIB094 REASON Phase 1 - NCT03976349 | clinical_registry | - | - | - | https://clinicaltrials.gov/study/NCT03976349 |
| LRRK2-targeting antisense oligonucleotide in Parkinson's disease | peer_reviewed | - | - | - | https://www.nature.com/articles/s41591-026-04262-4 |
| FinsnoBio LRRK2 Type II Pipeline | official_company | - | - | - | https://www.finsnobio.com/pipeline |
| Evaluation of c-Abl inhibitor vodobatinib in early Parkinson's disease | peer_reviewed | - | - | - | https://www.nature.com/articles/s41531-026-01275-1 |
| BIIB122 Phase 2a BEACON - NCT06602193 | clinical_registry | - | - | - | https://clinicaltrials.gov/study/NCT06602193 |


### MoA Validity

| Field | Value |
|---|---|
| Score | 3 |
| Evidence Type | E3_asset_specific_preclinical_or_technical_evidence |
| Evidence Type Reason | Direct biochemical inhibition and mechanism-linked cellular biomarkers are demonstrated for both LRRK2 and c-Abl. |
| Rubric Definition | - |
| Judgment Reason | Strong direct preclinical validation of intended dual-target pharmacology. |
| Why Not Higher | Score 3 is the maximum; LRRK2 Type II binding itself has not yet been structurally confirmed for the assessed compounds. |
| Investigation Note | Do not interpret the c-Abl co-crystal as direct structural proof of the LRRK2 binding mode. |

#### Conflicting Or Missing Evidence

- LRRK2 cryo-EM structural confirmation was planned rather than completed in the reviewed deck.


#### Evidence Sources

| Source | Type | Reliability | Supports Score | Evidence Summary | URL |
|---|---|---|---|---|---|
| [104] Current Progress and Key Findings_NCDP.pdf | user_uploaded_file | - | - | - | - |
| c-Abl phosphorylates alpha-synuclein and regulates its degradation | peer_reviewed | - | - | - | https://pubmed.ncbi.nlm.nih.gov/24412932/ |
| Phos-tag analysis of Rab10 phosphorylation by LRRK2 | peer_reviewed | - | - | - | https://pubmed.ncbi.nlm.nih.gov/27474410/ |
| Type II kinase inhibitors that target Parkinson's disease-associated LRRK2 | peer_reviewed | - | - | - | https://pubmed.ncbi.nlm.nih.gov/40465731/ |


### Platform Attractiveness

| Field | Value |
|---|---|
| Score | 1 |
| Evidence Type | E1_company_claim_or_scientific_rationale_only |
| Evidence Type Reason | A reusable AI-enabled drug discovery engine is disclosed and used, but quantitative technical superiority is not benchmarked. |
| Rubric Definition | - |
| Judgment Reason | Reusable AI/medchem infrastructure is credible, but differentiated technical performance is not quantitatively demonstrated. |
| Why Not Higher | No platform-vs-comparator quantitative result on hit rate, cycle time, quality or cost. |
| Investigation Note | Asset potency and PK are not double-counted as evidence of platform superiority. |

#### Conflicting Or Missing Evidence

- Magnitude of AI contribution versus conventional medicinal chemistry is unknown.


#### Evidence Sources

| Source | Type | Reliability | Supports Score | Evidence Summary | URL |
|---|---|---|---|---|---|
| 1ST Biotherapeutics R&D Pipeline | official_company | - | - | - | https://www.1stbio.com/rd-pipeline/ |
| [104] Current Progress and Key Findings_NCDP.pdf | user_uploaded_file | - | - | - | - |


### Expansion Potential

| Field | Value |
|---|---|
| Score | 0 |
| Evidence Type | E0_not_found_or_not_assessable |
| Evidence Type Reason | No additional active indication tied specifically to 1ST-104 was verified. |
| Rubric Definition | - |
| Judgment Reason | No confirmed asset-specific indication expansion beyond Parkinson's disease. |
| Why Not Higher | No additional indication with explicit 1ST-104 biological rationale or program status was verified. |
| Investigation Note | Other 1STBIO neurodegeneration programs are separate assets and cannot be counted as 1ST-104 expansion. |

#### Conflicting Or Missing Evidence

- None


#### Evidence Sources

| Source | Type | Reliability | Supports Score | Evidence Summary | URL |
|---|---|---|---|---|---|
| 1ST Biotherapeutics R&D Pipeline | official_company | - | - | - | https://www.1stbio.com/rd-pipeline/ |
| [104] Current Progress and Key Findings_NCDP.pdf | user_uploaded_file | - | - | - | - |


### Data Maturity

| Field | Value |
|---|---|
| Score | 3 |
| Evidence Type | E3_asset_specific_preclinical_or_technical_evidence |
| Evidence Type Reason | Multiple complementary quantitative domains include target pharmacology, cellular biomarkers, selectivity, ADME and CNS PK. |
| Rubric Definition | - |
| Judgment Reason | Broad and quantitative lead-optimization package across several development questions. |
| Why Not Higher | Score 3 is the maximum; candidate nomination, completed in vivo efficacy and development-grade toxicology remain outstanding. |
| Investigation Note | hERG and lung safety require particular follow-up before candidate selection is considered de-risked. |

#### Conflicting Or Missing Evidence

- No completed 1ST-104 in vivo efficacy package was identified.
- FB-4853 repeat-dose lung toxicity results were not present in the reviewed attachment.
- FB-4853 hERG IC50 is 8.75 micromolar.


#### Evidence Sources

| Source | Type | Reliability | Supports Score | Evidence Summary | URL |
|---|---|---|---|---|---|
| [104] Current Progress and Key Findings_NCDP.pdf | user_uploaded_file | - | - | - | - |


### Marketability

| Field | Value |
|---|---|
| Score | 0 |
| Evidence Type | E0_not_found_or_not_assessable |
| Evidence Type Reason | No reliable asset-specific forecast and insufficient evidence for a non-speculative A/B/C/D calculation. |
| Rubric Definition | - |
| Judgment Reason | Commercial opportunity may be large, but asset-specific obtainable peak sales cannot yet be reliably quantified. |
| Why Not Higher | No reliable calculation or asset-specific external forecast supports a sales threshold. |
| Investigation Note | Broad versus LRRK2-driven patient selection materially changes TAP and competition assumptions after the LUMA result. |

#### Conflicting Or Missing Evidence

- Commercial target population is not defined.
- No appropriate approved disease-modifying benchmark price was identified.
- No asset-specific 1ST-104 peak-sales forecast was verified.

#### Marketability A/B/C Calculation

| Step | Inputs / Assumptions | Formula | Output |
|---|---|---|---|
| A. TAP | Total patients: -; Diagnosis: -; Eligibility: -; Biomarker: -; Treatable subgroup: - | US TAP = US Patient Pool x Diagnosis Rate x Eligibility Rate x Treatable Subgroup Rate | - |
| B. Unrisked Peak Sales | TAP: -; Annual net price: -; Peak penetration: -; Duration factor: -; Entry/share: - | US Unrisked Peak Sales = US TAP x Benchmark Annualized Net Price x Peak Penetration x Treatment Duration Factor | - |
| C. Obtainable Peak Sales | Competition haircut: -; Pricing power: -; Expansion: - | US Obtainable Peak Sales = US Unrisked Peak Sales x Competition Haircut x Pricing Power Adjustment | - |


#### Evidence Sources

| Source | Type | Reliability | Supports Score | Evidence Summary | URL |
|---|---|---|---|---|---|
| BIIB122 Phase 2b LUMA Update | company_press_release | - | - | - | https://investors.biogen.com/news-releases/news-release-details/biogen-and-denali-therapeutics-provide-update-phase-2b-luma |
| Parkinson's Foundation Prevalence and Incidence | patient_foundation | - | - | - | https://www.parkinson.org/understanding-parkinsons/statistics/prevalence-incidence |
| BIIB122 Phase 2a BEACON - NCT06602193 | clinical_registry | - | - | - | https://clinicaltrials.gov/study/NCT06602193 |


## Competitive Similarity

| Metric | Value |
|---|---:|
| Similar pipelines | 7 |
| High similarity | 4 |
| Medium similarity | 3 |
| Low similarity | 0 |



| Competitor | Company | Modality | Target or MoA | Stage | Relevance | Source |
|---|---|---|---|---|---|---|
| NEU-411 | Neuron23 | Small molecule | Selective LRRK2 kinase inhibitor | Phase 2 | Direct oral LRRK2 competitor in genetically selected LRRK2-driven early Parkinson's disease. | https://clinicaltrials.gov/study/NCT06680830 |
| BIIB122 / DNL151 | Denali Therapeutics / Biogen | Small molecule | LRRK2 kinase inhibitor | Phase 2a BEACON active; idiopathic PD development discontinued after Phase 2b LUMA | Direct target/modality benchmark and major negative efficacy precedent in broad early PD. | https://investors.biogen.com/news-releases/news-release-details/biogen-and-denali-therapeutics-provide-update-phase-2b-luma |
| BT-267 | Brenig Therapeutics | Small molecule | Brain-penetrant selective LRRK2 inhibitor | Phase 1 | Direct oral CNS LRRK2 competitor emphasizing selectivity and brain exposure. | https://www.brenigther.com/ |
| Type II LRRK2 program | FinsnoBio | Small molecule | Type II and G2019S-focused LRRK2 inhibition | Preclinical / IND-directed; exact asset stage undisclosed | Directly competes with the Type II LRRK2 differentiation concept. | https://www.finsnobio.com/pipeline |
| ARV-102 | Arvinas | PROTAC | LRRK2 protein degradation | Phase 1 PD data | Alternative LRRK2 modality with demonstrated human CNS target modulation. | https://ir.arvinas.com/news-releases/news-release-details/arvinas-announces-positive-phase-1-data-arv-102-showing-greater/ |
| BIIB094 | Ionis / Biogen | RNA therapy | LRRK2-targeting antisense oligonucleotide | Phase 1 completed | Clinical human evidence for direct LRRK2 lowering through a distinct modality. | https://www.nature.com/articles/s41591-026-04262-4 |
| Vodobatinib | SPARC | Small molecule | c-Abl kinase inhibitor | Phase 2 completed; negative efficacy | Clinical negative precedent for the c-Abl component of 1ST-104's dual mechanism. | https://www.nature.com/articles/s41531-026-01275-1 |

| Company | Asset | Similarity | Matched dimensions | Shared data points |
|---|---|---:|---|---|
| Neuron23 | NEU-411 | 3 | Parkinson's disease, LRRK2, Small molecule | Brain penetration<br>LRRK2 pathway inhibition |
| Denali Therapeutics / Biogen | BIIB122 / DNL151 | 3 | Parkinson's disease, LRRK2, Small molecule | pS935 biomarker<br>pRab10 biomarker<br>Oral CNS-directed pharmacology |
| Brenig Therapeutics | BT-267 | 3 | Parkinson's disease, LRRK2, Small molecule | Brain penetration<br>Kinase selectivity |
| FinsnoBio | Type II LRRK2 program | 3 | Parkinson's disease, LRRK2, Small molecule, Type II pharmacology | Type II binding concept<br>Potential lung-safety differentiation |
| Arvinas | ARV-102 | 2 | Parkinson's disease, LRRK2 pathway | CNS target modulation<br>Neuroinflammatory biomarkers |
| Ionis / Biogen | BIIB094 | 2 | Parkinson's disease, LRRK2 pathway | Direct LRRK2 modulation |
| SPARC | Vodobatinib | 2 | Parkinson's disease, c-Abl pathway | Brain-penetrant small molecule<br>c-Abl inhibition |

### Competitor Evidence Sources

| Source | Type | Reliability | Supports Score | Evidence Summary | URL |
|---|---|---|---|---|---|
| None | - | - | - | - | - |

## Key Strengths

- None

## Key Risks

- None

## Validation

### Cross-Checked Facts

- 1ST-104 identity, LRRK2/c-Abl target and Parkinson's indication are supported by the official 1STBIO pipeline and supplied company deck.
- Active lead-optimization work is documented in the March 2026 company deck while the current public pipeline remains active.
- LRRK2 Parkinson's disease relevance is independently supported by human genetics.
- NEU-411 Phase 2 and BIIB122 Phase 2a competitor statuses were checked against ClinicalTrials.gov and sponsor sources.

### Uncertain Points

- Formal 1ST-104 development-candidate nomination has not been verified.
- Direct structural confirmation of Type II binding to LRRK2 remains unverified in the supplied evidence.
- Completed asset-specific in vivo efficacy and repeat-dose lung toxicity data were not found in the reviewed deck.
- FB-4853A hERG IC50 of 8.75 micromolar requires exposure-margin assessment.
- Broad versus LRRK2-driven Parkinson's patient-selection strategy is unresolved.
- No reliable asset-specific peak-sales forecast is available.

## Source Links

- Company: [[1ST Biotherapeutics, Inc]]
- Theme: [[Theme - Neuroimmune|Neuroimmune]]
- Cluster: [[Cluster - Cytokine 신경조절|Cytokine 신경조절]]
- Dashboard record id: `1ST Biotherapeutics, Inc._1ST-104`

## Raw JSON

```json
{
  "meta": {
    "review_type": "full_scout",
    "schema_version": "3.2",
    "instruction_version": "3.4",
    "rubric_version": "3.4",
    "generated_at": "2026-08-18",
    "output_filename_base": "1ST Biotherapeutics, Inc._1ST-104",
    "storage_profile": "dashboard_hybrid_v1"
  },
  "source_report": {
    "raw_markdown": "# 1ST Biotherapeutics Pipeline Scout Report: **1ST-104**\n\n본 보고서는 GPT instruction 2 — Full Scout v3.4 (schema v3.2)에 따라 attachment review, public-source verification, competitor search 및 7개 criterion scoring을 수행했으며, auditability를 위해 확인한 URL을 포함합니다. :contentReference[oaicite:0]{index=0}\n\n**중요: 1ST-104는 공식 pipeline과 제공자료에서 Parkinson's disease용 LRRK2/c-Abl dual inhibitor 프로그램으로 확인되고 active lead-optimization program으로 판단되나, 총점 12/21로 REVIEW / Watch입니다. Preclinical target/PK profile은 흥미롭지만 candidate가 아직 공식 nomination되지 않았고, in vivo efficacy·1ST-104 자체 반복독성·human data가 없으며, 최근 LRRK2 및 c-Abl 임상 실패 사례와 시장성 산정 불확실성이 남아 있습니다.**\n\n---\n\n## 1) Company Profile\n\n| Field | Content | Evidence |\n|---|---|---|\n| Company | 1ST Biotherapeutics, Inc. | [1], [3] |\n| Legal name / aliases | 1ST Biotherapeutics, Inc. / 1STBIO / 주식회사 퍼스트바이오테라퓨틱스 | [3] |\n| Country | Republic of Korea | [3] |\n| Headquarters | Yongin-si, Gyeonggi-do, Republic of Korea | [3] |\n| Website | https://www.1stbio.com/ | [3] |\n| Company type / stage | Private, clinical-stage biopharmaceutical company | [2], [3] |\n| Focus areas | Neurodegenerative diseases, oncology, rare diseases | [2] |\n| Platform summary | 1ST-501 AI Drug Discovery Engine을 보유하며, 1ST-104 자료에서는 AI-driven in-house discovery를 활용해 671개 이상의 compound를 합성하고 multiple lead series를 도출했다고 설명. 다만 platform 자체의 quantitative superiority versus conventional discovery는 제시되지 않음. | [1], attachment |\n| Financing / partnership signals | 2026-01 Series D 317억원 조달, 누적 투자금 약 1,080억원. MJFF LITE consortium 참여를 통한 LRRK2 연구 협력도 확인됨. | [2], [4] |\n| Lead pipeline summary | Oncology 및 neurodegeneration에서 clinical/preclinical pipeline을 운영. 1ST-104는 공식 pipeline상 LRRK2/c-Abl → Parkinson's disease program으로 등재됨. | [1] |\n\n1STBIO 공식 홈페이지는 Yongin 소재와 company identity를 확인하며, 공식 pipeline에는 1ST-104가 LRRK2/c-Abl을 표적하는 Parkinson's disease program으로 현재 등재되어 있습니다. :contentReference[oaicite:1]{index=1} 2026년 1월 회사는 317억원 Series D 및 누적 약 1,080억원의 투자 유치를 발표했습니다. :contentReference[oaicite:2]{index=2}\n\nAttachment에서는 671 compounds를 합성하고 multiple lead series가 lead optimization 단계에서 진행되고 있다고 명시합니다. :contentReference[oaicite:3]{index=3}\n\n---\n\n## 2) Pipeline Snapshot\n\n| Field | Content | Evidence |\n|---|---|---|\n| Company | 1ST Biotherapeutics, Inc. | [1], [3] |\n| Lead asset | 1ST-104; representative optimized compounds include FB-3538A and FB-4853A. Final development candidate is not yet confirmed in reviewed evidence. | Attachment |\n| Target | LRRK2 / c-Abl | [1], attachment |\n| Theme / Cluster | **Theme: Neuroimmune / Cluster: Cytokine 신경조절** | Asset-specific iPSC-derived microglia target/ cytokine experiments + LRRK2 biology; internal SKBP mapping |\n| MoA | Brain-penetrant dual LRRK2/c-Abl kinase inhibition. Lead series suppresses pRab10 and pCrkL while relatively preserving LRRK2 pS935, supporting a Type II-like LRRK2 pharmacology. Type II binding is structurally confirmed for c-Abl by co-crystal; LRRK2 structural confirmation was still planned. | Attachment, [5]-[8] |\n| Modality / Platform | Small molecule | [1], attachment |\n| Indication | Parkinson's disease | [1] |\n| Stage | **Lead Optimization**. Official public pipeline uses broader “Discovery” labeling; the March 2026 attachment specifically states active lead optimization. Candidate nomination in 3Q 2026 was a plan, not a completed milestone. | [1], attachment |\n| Key data | FB-4853A: LRRK2 biochemical IC50 11.12 nM; c-Abl IC50 0.93 nM; stimulated pRab10 IC50 12.41 nM; unstimulated pRab10 IC50 44.73 nM; pS935 IC50 >5,000 nM. Mouse PO F 62.08%, Kp,brain 0.91, Kp,uu,brain 0.31. hERG IC50 8.75 μM is a liability requiring follow-up. | Attachment |\n\n공식 pipeline에서 1ST-104의 target/indication은 LRRK2/c-Abl 및 Parkinson's disease로 명확히 확인됩니다. :contentReference[oaicite:4]{index=4} 제공자료에서는 candidate selection을 향한 lead optimization이 진행 중이며 candidate nomination은 3Q 2026, Phase I initiation은 3Q 2027의 **계획**으로 표시되어 있어 이를 완료된 milestone으로 취급하지 않았습니다. :contentReference[oaicite:5]{index=5}\n\n대표 compound FB-4853A는 LRRK2 11.12 nM 및 c-Abl 0.93 nM biochemical potency와 12.41 nM stimulated pRab10 potency를 보였습니다. :contentReference[oaicite:6]{index=6} :contentReference[oaicite:7]{index=7} Unstimulated A549 assay에서는 pRab10 IC50 44.73 nM인 반면 pS935 IC50가 >5,000 nM으로 나타나 Type II-like profile을 지지했습니다. :contentReference[oaicite:8]{index=8} :contentReference[oaicite:9]{index=9}\n\nMouse PK에서 FB-4853A는 5 mg/kg PO 기준 F 62.08%, Kp,brain 0.91, Kp,uu,brain 0.31을 보였습니다. :contentReference[oaicite:10]{index=10} 반면 hERG IC50는 8.75 μM으로, 자료 내 자체 기준상 10 μM 미만에 해당하여 development candidate 선정 시 exposure margin 확인이 중요한 safety diligence item입니다. :contentReference[oaicite:11]{index=11}\n\n---\n\n## 3) Scorecard Summary\n\n| Criterion | Score (maximum 3 points each) | One-line judgment | Evidence used |\n|---|---:|---|---|\n| Target Relevance | **3점** | PD는 SKBP interest indication이며 LRRK2는 human genetics로 직접 연결되고, 1ST-104는 LRRK2/c-Abl을 실제 pharmacology로 조절함. | [1], [5]-[7], attachment |\n| Competitive Landscape | **2점** | Type II-like/pS935-preserving profile이라는 quantitative differentiation은 있으나 NEU-411, BIIB122, BT-267 등 clinical LRRK2 competitors가 앞서 있으며 LUMA 및 c-Abl clinical failures가 존재. | [9]-[17], attachment |\n| MoA Validity | **3점** | Asset-specific biochemical potency, pRab10/pCrkL cellular PD, pS935 conformational profile 및 c-Abl co-crystal로 직접 기능 검증. | Attachment, [7], [8] |\n| Platform Attractiveness | **1점** | AI discovery platform과 반복 가능한 chemistry workflow는 확인되나 platform-vs-comparator quantitative advantage는 없음. | [1], attachment |\n| Expansion Potential | **0점** | 1ST-104 자체의 PD 외 추가 active indication/program은 확인되지 않음. | [1], attachment |\n| Data Maturity | **3점** | Biochemical/cellular pharmacology + selectivity/ADME/brain PK + microglia data 등 복수 quantitative domain이 존재. | Attachment |\n| Marketability | **0점** | Asset-specific forecast가 없고, broad PD vs biomarker-selected PD label과 적절한 DMT price benchmark가 확정되지 않아 신뢰 가능한 A/B/C/D 계산을 수행하지 않음. | [10], [18] |\n| **Total** | **12점** | Maximum total: 21점 → **REVIEW** | |\n\n---\n\n## 4) Criterion Detail Pages\n\n### 4.1 Target Relevance\nScore: **3점**\n\nMain line: **Parkinson's disease는 SKBP의 핵심 interest indication이며, LRRK2는 human genetic causality가 가장 강한 PD target 중 하나이고 1ST-104는 LRRK2와 c-Abl을 동시에 실제 pharmacology로 조절합니다.**\n\nWhat was checked:\n- Target identity\n- Disease/biology relevance\n- SKBP Theme / Cluster fit\n- General neurodegeneration / neuroinflammation / epilepsy relevance\n\nEvidence trail:\n- 공식 1STBIO pipeline: 1ST-104 = LRRK2/c-Abl, Parkinson's disease. [1] :contentReference[oaicite:12]{index=12}\n- LRRK2 disease-segregating mutations were originally identified in autosomal-dominant Parkinsonism, providing direct human genetic linkage. [5] :contentReference[oaicite:13]{index=13}\n- c-Abl has experimentally been linked to α-synuclein phosphorylation/degradation and PD-related neurodegeneration. [6] :contentReference[oaicite:14]{index=14}\n- Attachment shows 1ST-104 compounds directly inhibit both LRRK2 and c-Abl and modulate their downstream biomarkers. :contentReference[oaicite:15]{index=15}\n\nInvestigation note:\n- **3점 rule:** SKBP interest indication이 확인되고 target/MoA가 해당 disease biology에 직접 연결됨.\n- PD와 LRRK2의 genetic linkage가 강하며 1ST-104 자체에서도 target modulation이 확인되어 3점.\n- 3점이 최고점. 다만 dual-target에서 c-Abl contribution이 임상적으로 추가 benefit을 제공하는지는 아직 입증되지 않음.\n\n### 4.2 Competitive Landscape\nScore: **2점**\n\nMain line: **1ST-104의 pS935-preserving Type II-like profile은 기존 Type I LRRK2 inhibition과 차별화 가능성이 있으나, clinical-stage LRRK2 경쟁이 이미 상당히 진행되었고 LRRK2 및 c-Abl monotherapy의 최근 efficacy failures가 진입장벽을 높였습니다.**\n\nWhat was checked:\n- Same disease competitors\n- Same target competitors\n- Same or similar MoA competitors\n- Front runner count\n- Approved / Phase 3 / clinical / preclinical status\n- ClinicalTrials.gov, official company pipelines, recent primary clinical publications\n- Type II LRRK2 competitors\n\nCompetitor table:\n\n| Competitor | Company | Modality | Target / MoA | Stage | Why it matters | Source |\n|---|---|---|---|---|---|---|\n| NEU-411 | Neuron23 | Small molecule | Selective LRRK2 inhibitor | Phase 2, recruiting | Most advanced active precision-LRRK2 competitor; genotype-selected LRRK2-driven early PD | [9] |\n| BIIB122 / DNL151 | Denali / Biogen | Small molecule | LRRK2 inhibitor | Phase 2a BEACON active in LRRK2-PD; idiopathic PD program discontinued | Direct target/modality comparator; Phase 2b LUMA failed primary/secondary endpoints in idiopathic PD | [10], [19] |\n| BT-267 | Brenig Therapeutics | Small molecule | Brain-penetrant selective LRRK2 inhibitor | Phase 1 | Direct oral small-molecule LRRK2 competitor with CNS/selectivity positioning | [11] |\n| FinsnoBio Type II LRRK2 program | FinsnoBio | Small molecule | Type II / G2019S-focused LRRK2 inhibition | Preclinical/IND-directed; exact asset stage undisclosed | Particularly relevant to 1ST-104's proposed Type II differentiation | [14] |\n| ARV-102 | Arvinas | PROTAC | LRRK2 degradation | Phase 1 PD data | Different modality but clinically demonstrated CNS LRRK2 modulation; >50% CSF degradation reported | [12] |\n| BIIB094 | Ionis/Biogen | Antisense oligonucleotide | LRRK2 lowering | Phase 1 completed | Clinical human validation of an alternative LRRK2-modulating modality | [13] |\n| Vodobatinib | SPARC | Small molecule | c-Abl inhibitor | Phase 2 completed; negative | Shares c-Abl component of 1ST-104; trial did not show clinical benefit and had tolerability/dropout issues | [17] |\n\nNEU-411 is currently in a randomized Phase 2 trial in genetically selected LRRK2-driven early PD. :contentReference[oaicite:16]{index=16} BIIB122's Phase 2b LUMA study in broader early PD failed its primary and secondary efficacy endpoints in May 2026; development in idiopathic PD was discontinued, although the LRRK2-variant BEACON Phase 2a remains active. :contentReference[oaicite:17]{index=17}\n\nAdditional clinical competition includes Brenig's BT-267 Phase 1 LRRK2 inhibitor and Arvinas' ARV-102, which reported ≥50% LRRK2 reduction in CSF in Phase 1 PD patients. :contentReference[oaicite:18]{index=18} FinsnoBio is independently developing Type II LRRK2 inhibitors, making the \"Type II\" concept itself non-exclusive competitively. :contentReference[oaicite:19]{index=19}\n\nFor c-Abl, the 2026 PROSEEK Phase 2 publication reported that vodobatinib did not demonstrate clinical benefit over placebo; this is relevant because c-Abl is the second pharmacologic component of 1ST-104. :contentReference[oaicite:20]{index=20}\n\nAsset-specific differentiation:\n- Internal DNL151 comparison showed DNL151 pRab10 IC50 46.14 nM and pS935 IC50 66.92 nM, whereas later 1ST-104 compound FB-4853A showed pRab10 IC50 44.73 nM with pS935 IC50 >5,000 nM. This supports substantially greater pS935 preservation at similar cellular pRab10 potency, consistent with Type II-like differentiation. :contentReference[oaicite:21]{index=21} :contentReference[oaicite:22]{index=22}\n- However, this is company-generated preclinical comparison, not externally reproduced or clinical head-to-head evidence.\n\nInvestigation note:\n- **2점 rule:** asset-specific quantitative differentiation versus an appropriate comparator or a credible differentiated entry space.\n- pRab10/pS935 data provide this threshold.\n- **Not 3:** multiple high-similarity clinical competitors are ahead; Type II LRRK2 is not unique; no human data or independent head-to-head evidence exists, and both broad LRRK2 inhibition and c-Abl inhibition have recent negative Phase 2 efficacy precedents.\n\n### 4.3 MoA Validity\nScore: **3점**\n\nMain line: **1ST-104 compounds directly engage both intended kinase pathways and show mechanism-linked cellular PD biomarkers plus a conformationally differentiated LRRK2 profile.**\n\nWhat was checked:\n- Journal publication / PMID / DOI\n- Mechanistic consistency\n- Functional readout\n- Disease linkage\n- Safety-relevant signal\n\nEvidence trail:\n- FB-4853A biochemical LRRK2 IC50 11.12 nM, c-Abl IC50 0.93 nM. :contentReference[oaicite:23]{index=23} :contentReference[oaicite:24]{index=24}\n- Cellular stimulated pRab10 IC50 12.41 nM and pCrkL IC50 0.67 nM. :contentReference[oaicite:25]{index=25} :contentReference[oaicite:26]{index=26}\n- FB-4853A retains 79.9% pS935 level at its pRab10 IC50 and 63.9% at pRab10 IC80, greater preservation than FB-3538A, supporting Type II-like LRRK2 behavior. :contentReference[oaicite:27]{index=27}\n- The deck explicitly summarizes potent pRab10 inhibition with preserved pS935 as a Type II-like binding profile. :contentReference[oaicite:28]{index=28}\n- c-Abl Type II binding was directly demonstrated by X-ray co-crystal; **LRRK2 cryo-EM confirmation was still planned**, so structural confirmation should not be extended from c-Abl to LRRK2. :contentReference[oaicite:29]{index=29}\n- Independent studies validate Rab10 phosphorylation as a direct LRRK2 kinase readout. [7] :contentReference[oaicite:30]{index=30}\n- Independent primary work has characterized Type II inhibitors that stabilize an inactive/open LRRK2 kinase conformation. [8] :contentReference[oaicite:31]{index=31}\n\nInvestigation note:\n- **3점 rule:** assessed-asset target engagement, mechanism-linked PD/biomarker or direct functional validation.\n- 1ST-104 meets this through biochemical LRRK2/c-Abl inhibition and downstream pRab10/pCrkL cellular effects.\n- 3점이 최고점. Remaining diligence is not MoA existence but whether the dual/Type II profile translates to superior in vivo efficacy and safety.\n\n### 4.4 Platform Attractiveness\nScore: **1점**\n\nMain line: **Reusable AI/medicinal-chemistry discovery infrastructure exists, but its claimed advantage has not been quantitatively benchmarked against an appropriate discovery comparator.**\n\nWhat was checked:\n- Is the platform real and reproducible?\n- Is differentiation supported by data?\n- Is the underlying technical system reusable across candidates, programs, or conditions?\n\nEvidence trail:\n- Official pipeline identifies **1ST-501 AI Drug Discovery Engine** as a drug-engineering platform integrating medicinal chemistry and AI. [1] :contentReference[oaicite:32]{index=32}\n- 1ST-104 attachment states that the AI-driven in-house platform contributed to synthesis of >671 compounds and identification of multiple lead series. :contentReference[oaicite:33]{index=33}\n- 671-compound output demonstrates substantial use of the system but does not quantify whether AI improved hit rate, cycle time, potency, selectivity, cost, or candidate success versus a proper comparator.\n\nInvestigation note:\n- **1점 rule:** reusable structure with plausible rationale but claim/concept-level differentiation.\n- **Not 2:** no direct quantitative platform-vs-comparator experiment or metric was provided.\n- The 1ST-104 potency/PK data are counted under Data Maturity and are not double-counted as platform superiority.\n\nPlatform vs Data Maturity separation:\n- Platform Attractiveness = reusable discovery system.\n- Data Maturity = 1ST-104 compound data.\n- The 671-compound campaign establishes platform use but not a quantitative technical advantage.\n\n### 4.5 Expansion Potential\nScore: **0점**\n\nMain line: **Reviewed official and company-supplied evidence confirms Parkinson's disease for 1ST-104 but does not confirm another active asset-specific indication.**\n\nWhat was checked:\n- Expansion beyond main indication\n- Asset-specific quantitative data in additional indications\n- Confirmed asset-specific preclinical, IND-enabling, or clinical development programs\n\nEvidence trail:\n- Official 1STBIO pipeline lists 1ST-104 only under Parkinson's disease. [1] :contentReference[oaicite:34]{index=34}\n- The supplied 1ST-104 development timeline is directed toward PD models and future healthy-volunteer/PD clinical work. :contentReference[oaicite:35]{index=35}\n\nInvestigation note:\n- **0점 rule:** no confirmed additional indication.\n- **Not 1:** biological plausibility for other neurodegenerative indications is not enough; an additional indication explicitly tied to 1ST-104 was not verified.\n- Other 1STBIO assets or historical c-Abl/LRRK2 programs are not treated as expansion of 1ST-104.\n\n### 4.6 Data Maturity\nScore: **3점**\n\nMain line: **Lead-optimization stage에 적절한 quantitative evidence가 biochemical/cellular pharmacology, selectivity, ADME/PK/brain penetration 및 human iPSC-derived microglia 등 여러 독립적 development questions를 커버합니다.**\n\nWhat was checked:\n- In vitro data\n- In vivo data\n- Quantitative result\n- Reproducibility\n- IND-enabling / GLP tox / PK/PD / CMC / human data availability\n\nEvidence trail:\n\n**Domain 1 — Target pharmacology / cellular PD**\n- FB-4853A LRRK2 11.12 nM, c-Abl 0.93 nM.\n- Cellular pRab10 and pCrkL inhibition demonstrated.\n- pRab10 reduction with relative pS935 preservation supports intended Type II-like profile.\n:contentReference[oaicite:36]{index=36} :contentReference[oaicite:37]{index=37}\n\n**Domain 2 — Selectivity**\n- FB-4853A: PDGFRα >2,500 nM, KDR 3,797.8 nM, c-Kit >12,500 nM, LCK 4,314 nM, RIPK2 >5,000 nM.\n- Residual liabilities include DYRK1A 144.1 nM and TRKB 511 nM, which should remain in the selectivity diligence package.\n:contentReference[oaicite:38]{index=38} :contentReference[oaicite:39]{index=39}\n\n**Domain 3 — PK / CNS exposure**\n- FB-4853A 5 mg/kg PO: T1/2 1.37 h, Cmax 1,422.13 ng/mL, AUClast 2,301.69 h·ng/mL, F 62.08%, Kp,brain 0.91, Kp,uu,brain 0.31.\n:contentReference[oaicite:40]{index=40}\n\n**Domain 4 — Human iPSC-derived microglia**\n- PFF/IFN-γ-stimulated healthy iPSC-derived microglia experiments evaluated LRRK2 pS935, Rab10 pT73 and cytokines TNF-α, IL-1β and IL-6 with FB-4853A.\n- Cytokine reduction was described, but the deck explicitly states stimulation-condition optimization was still ongoing, so efficacy strength should not be overstated.\n:contentReference[oaicite:41]{index=41} :contentReference[oaicite:42]{index=42}\n\n**Safety / liability**\n- FB-4853A hERG IC50 8.75 μM. :contentReference[oaicite:43]{index=43}\n- A planned 7-day FB-4853 mouse lung toxicity study is shown, but the reviewed attachment contains the design rather than completed FB-4853 results. :contentReference[oaicite:44]{index=44}\n\nInvestigation note:\n- **3점 rule:** at least two complementary quantitative evidence domains addressing different development questions, with at least one supporting program progression.\n- 1ST-104 clearly exceeds this threshold.\n- 3점이 최고점. Nevertheless, candidate nomination, completed in vivo efficacy/PK-PD, compound-specific repeat-dose lung tox and GLP tox are still required before development risk can be considered low.\n\n### 4.7 Marketability\nScore: **0점**\n\nMain line: **Commercial potential cannot yet be translated into a reliable asset-specific peak-sales estimate without speculative assumptions.**\n\nAssessment method: **insufficient_evidence**\n\nScore basis type: **insufficient_evidence**\n\nCalculation status: **not_performed**\n\nAssessed global peak sales (million USD): **not assessed**\n\nWhat was checked:\n- Internal A/B/C/D calculation\n- Reliable asset-specific external peak-sales forecast\n- Competition haircut and pricing power\n- Current PD epidemiology\n- Likely broad versus biomarker-selected patient positioning after contemporary LRRK2 clinical results\n\nWorksheet:\n\n| Step | What to fill | Evidence / assumption |\n|---|---|---|\n| A. US TAP | Not performed | U.S. PD prevalence is large, but the clinically relevant 1ST-104 population is unresolved: broad idiopathic PD vs genetically/biomarker-selected LRRK2-driven PD. |\n| B. US Unrisked Peak Sales | Not performed | No appropriate approved disease-modifying oral LRRK2/c-Abl benchmark price was identified. Using a symptomatic PD therapy would introduce an arbitrary benchmark. |\n| C. US Obtainable Peak Sales | Not performed | Competition haircut cannot be reliably fixed before intended patient selection, differentiation and timing versus NEU-411/other LRRK2 approaches are established. |\n| D. Global Obtainable Peak Sales | Not performed | C is unavailable; therefore the user-defined 1.5x multiplier is not applied. |\n| External Peak Sales Reference | None verified | No reliable asset-specific 1ST-104 peak-sales forecast was found in reviewed public sources. |\n| Final score basis | 0 | Neither a reliable internal calculation nor asset-specific external forecast exists. |\n\nThe Parkinson's Foundation reports a U.S. disease burden of roughly one million people living with PD and nearly 90,000 new diagnoses annually, confirming a large disease population. :contentReference[oaicite:45]{index=45} However, the May 2026 LUMA failure in broad early-stage PD and continued development of BIIB122 in genetically defined LRRK2-PD make **patient selection a central commercial variable**, rather than a detail that can be safely assumed. :contentReference[oaicite:46]{index=46}\n\nInvestigation note:\n- Marketability is not scored from disease prevalence alone.\n- No A/B/C/D calculation was forced because treatable subgroup, valid benchmark annualized net price and competition haircut would all require unsupported analyst assumptions.\n- **Not 1:** no reliable global obtainable peak-sales estimate below or above USD 1,000 million can currently be established.\n\n---\n\n## 5) Validation Notes\n\nCross-checked facts:\n- **Asset identity:** 1ST-104 is independently visible on 1STBIO's current official pipeline as LRRK2/c-Abl for Parkinson's disease and is also the subject of the supplied March 2026 development deck. [1] :contentReference[oaicite:47]{index=47} :contentReference[oaicite:48]{index=48}\n- **Active program:** the public pipeline remains live, while the supplied deck documents active lead optimization and ongoing planned development work. :contentReference[oaicite:49]{index=49}\n- **LRRK2 target validity:** human genetic evidence links pathogenic LRRK2 mutations directly to Parkinsonism, and multiple independent clinical programs remain directed at LRRK2. [5], [9], [10] :contentReference[oaicite:50]{index=50}\n- **Competitive signal:** NEU-411 Phase 2 status is confirmed by ClinicalTrials.gov; BIIB122 LUMA failure and ongoing BEACON are confirmed by sponsor/registry sources. :contentReference[oaicite:51]{index=51}\n- **Company status:** current official company pages and the 2026 Series D release support Republic of Korea domicile, Yongin HQ and ongoing financing/business activity. :contentReference[oaicite:52]{index=52}\n\nUncertain points:\n- **Development candidate:** FB-3538A and FB-4853A are representative optimized compounds, but no reviewed source confirms which compound has been formally nominated as the 1ST-104 development candidate.\n- **Stage milestone:** candidate nomination in 3Q 2026 was a forward-looking plan in the March deck. No public source reviewed as of 2026-08-18 confirms that nomination has actually occurred.\n- **LRRK2 structural mode:** c-Abl Type II binding is confirmed by co-crystal, while direct LRRK2 Type II structural confirmation by cryo-EM was still planned. Do not equate the two.\n- **In vivo efficacy:** the supplied deck contains planned in vivo biomarker/efficacy studies but not a completed 1ST-104 in vivo efficacy package sufficient to establish disease modification.\n- **Lung safety:** the attachment demonstrates the MLi-2 positive-control lung phenotype and describes a planned FB-4853 comparison; completed FB-4853 repeat-dose lung histopathology is not present in the reviewed deck.\n- **hERG:** FB-4853A IC50 8.75 μM requires exposure-margin assessment once the development candidate and projected human exposure are selected.\n- **Patient-selection strategy:** broad idiopathic PD versus genetically/biomarker-defined LRRK2-driven PD is not established in the reviewed 1ST-104 materials. This is particularly important after the negative BIIB122 LUMA readout.\n- **Commercial forecast:** no credible asset-specific peak-sales forecast was verified.\n\nSearch log:\n- Official company page: checked [3]\n- Pipeline page: checked [1]\n- Platform page: checked platform section of [1]\n- Publications: checked LRRK2 genetics, Rab10 biology, c-Abl/α-synuclein, Type II LRRK2 and c-Abl Phase 2 literature [5]-[8], [17]\n- Regulatory / trial registry: NEU-411, BIIB122, BIIB094 [9], [13], [19]\n- Competitor sources: Neuron23, Biogen/Denali, Brenig, Arvinas, Ionis/Biogen, FinsnoBio and vodobatinib [9]-[17], [19]\n- Market / epidemiology sources: Parkinson's Foundation [18]; no asset-specific external sales forecast found\n- Financing / partnership sources: 1STBIO Series D and MJFF LITE releases [2], [4]\n\n---\n\n## 6) Final Take\n\nOne-line summary:\n\n**1ST-104는 PD target relevance와 preclinical pharmacology/brain PK가 강하고 pRab10 inhibition–pS935 preservation이라는 Type II-like differentiation이 매력적이지만, 아직 candidate nomination·in vivo efficacy·compound-specific repeat-dose safety가 완성되지 않았고, LRRK2/c-Abl 경쟁 및 최근 임상 실패를 감안하면 현재는 Shortlist보다는 Watch가 적절합니다.**\n\nRecommendation:\n- **Watch**\n\nMost important diligence question:\n\n**“최종 candidate에서 충분한 CNS target coverage를 확보하면서 FB-4853 계열의 hERG 및 LRRK2-related lung liability를 해소하고, BIIB122 LUMA와 c-Abl inhibitor Phase 2 failures를 넘어서는 in vivo efficacy/biomarker signal을 보여줄 수 있는가? 또한 개발 대상 환자를 broad idiopathic PD가 아닌 LRRK2-driven biomarker-selected population으로 정의할 것인가?”**\n\n---\n\n## References\n\nAttachment source: **[104] Current Progress and Key Findings_NCDP.pdf, 1ST Biotherapeutics, March 2026.** :contentReference[oaicite:53]{index=53}\n\n[1]: https://www.1stbio.com/rd-pipeline/ \"1ST Biotherapeutics R&D Pipeline\"\n[2]: https://www.1stbio.com/wp-content/uploads/2025/04/1st-biotherapeutics-joins-the-michael-j-fox.pdf \"1ST Biotherapeutics Joins the Michael J. Fox Foundation LITE Program\"\n[3]: https://www.1stbio.com/ \"1ST Biotherapeutics official website\"\n[4]: https://www.1stbio.com/wp-content/uploads/2026/01/1STBIO_Series_D_completed.pdf \"1STBIO Series D financing announcement\"\n[5]: https://pubmed.ncbi.nlm.nih.gov/15541309/ \"Mutations in LRRK2 cause autosomal-dominant parkinsonism\"\n[6]: https://pubmed.ncbi.nlm.nih.gov/24412932/ \"c-Abl phosphorylates alpha-synuclein and regulates its degradation\"\n[7]: https://pubmed.ncbi.nlm.nih.gov/27474410/ \"Phos-tag analysis of Rab10 phosphorylation by LRRK2\"\n[8]: https://pubmed.ncbi.nlm.nih.gov/40465731/ \"Type II kinase inhibitors that target Parkinson's disease-associated LRRK2\"\n[9]: https://clinicaltrials.gov/study/NCT06680830 \"NEU-411 Phase 2 NEULARK\"\n[10]: https://investors.biogen.com/news-releases/news-release-details/biogen-and-denali-therapeutics-provide-update-phase-2b-luma \"BIIB122 Phase 2b LUMA update\"\n[11]: https://www.brenigther.com/ \"Brenig Therapeutics BT-267\"\n[12]: https://ir.arvinas.com/news-releases/news-release-details/arvinas-announces-positive-phase-1-data-arv-102-showing-greater/ \"ARV-102 Phase 1 PD data\"\n[13]: https://www.nature.com/articles/s41591-026-04262-4 \"LRRK2-targeting antisense oligonucleotide in Parkinson's disease\"\n[14]: https://www.finsnobio.com/pipeline \"FinsnoBio LRRK2 Type II program\"\n[15]: https://neuron23.com/pipeline/ \"Neuron23 NEU-411 pipeline\"\n[16]: https://clinicaltrials.gov/study/NCT03976349 \"BIIB094 Phase 1 REASON study\"\n[17]: https://www.nature.com/articles/s41531-026-01275-1 \"Vodobatinib Phase 2 PROSEEK study\"\n[18]: https://www.parkinson.org/understanding-parkinsons/statistics/prevalence-incidence \"Parkinson's Foundation prevalence and incidence\"\n[19]: https://clinicaltrials.gov/study/NCT06602193 \"BIIB122 Phase 2a BEACON study\"",
    "parser_status": "gpt_structured_output"
  },
  "input": {
    "company_input": "1ST Biotherapeutics",
    "asset_input": "1ST-104"
  },
  "json_summary": {
    "theme": "Neuroimmune",
    "cluster": "Cytokine 신경조절",
    "target_description": "Brain-penetrant small-molecule dual LRRK2/c-Abl inhibition with a Type II-like LRRK2 pharmacology."
  },
  "structured_table": {
    "company": "1ST Biotherapeutics, Inc.",
    "asset_name": "1ST-104",
    "target": "LRRK2 / c-Abl",
    "moa": "Dual LRRK2/c-Abl kinase inhibition with pRab10 and pCrkL suppression and relative pS935 preservation; c-Abl Type II co-crystal confirmed.",
    "modality_platform": "Small molecule",
    "main_indication": "Parkinson's disease",
    "indication": "Parkinson's disease",
    "development_stage": "Lead Optimization",
    "company_country": "Republic of Korea",
    "sources": [
      {
        "source_title": "1ST Biotherapeutics R&D Pipeline",
        "source_url": "https://www.1stbio.com/rd-pipeline/"
      }
    ]
  },
  "hard_filter": {
    "status": "REVIEW",
    "reason": "Total score is 12/21, within the REVIEW range. Asset identity and active development are verified, but candidate nomination, in vivo efficacy, compound-specific repeat-dose safety and commercial evidence remain immature.",
    "flags": [
      "Total score 12/21",
      "No human data for 1ST-104",
      "Development candidate not yet verified as nominated",
      "Marketability not reliably assessable"
    ],
    "hard_blocker": false,
    "decision_uncertainty": false
  },
  "scoring": {
    "criteria": {
      "target_relevance": {
        "score": 3,
        "evidence_type": "E3_asset_specific_preclinical_or_technical_evidence",
        "evidence_type_reason": "1ST-104 is confirmed as an LRRK2/c-Abl Parkinson's program with direct asset-specific target pharmacology, supported by independent PD target biology.",
        "evidence_basis": "Parkinson's disease is an SKBP interest indication; LRRK2 has direct human genetic linkage to PD.",
        "main_line_summary": "Strong PD fit with genetically validated LRRK2 biology and asset-specific dual-target activity.",
        "why_not_higher": "Score 3 is the maximum; clinical proof of benefit for the dual-target strategy is still absent.",
        "investigation_note": "LRRK2 provides the strongest disease-biology anchor; c-Abl adds mechanistic rationale but weaker clinical validation.",
        "uncertain_points": [
          "Incremental clinical contribution of c-Abl inhibition is unproven."
        ],
        "source_ids": [
          "S1",
          "S3",
          "S5",
          "S14"
        ]
      },
      "competitive_landscape": {
        "score": 2,
        "evidence_type": "E3_asset_specific_preclinical_or_technical_evidence",
        "evidence_type_reason": "Internal quantitative pRab10/pS935 comparison supports differentiation versus a Type I comparator, while multiple clinical LRRK2 programs define a crowded field.",
        "evidence_basis": "Comparable pRab10 inhibition with greater pS935 preservation supports a Type II-like differentiated profile.",
        "main_line_summary": "Quantitative differentiation exists, but several clinical LRRK2 competitors are ahead and recent efficacy failures increase risk.",
        "why_not_higher": "No clinical or independently replicated head-to-head differentiation; multiple high-similarity programs are ahead.",
        "investigation_note": "LUMA and vodobatinib failures increase the importance of patient selection and proof that dual/Type II pharmacology changes efficacy.",
        "uncertain_points": [
          "Clinical relevance of pS935 preservation is not established.",
          "Exact entry timing versus Phase 2 competitors depends on candidate nomination and IND progression."
        ],
        "source_ids": [
          "S3",
          "S6",
          "S7",
          "S8",
          "S9",
          "S10",
          "S11",
          "S12",
          "S17",
          "S19"
        ]
      },
      "moa_validity": {
        "score": 3,
        "evidence_type": "E3_asset_specific_preclinical_or_technical_evidence",
        "evidence_type_reason": "Direct biochemical inhibition and mechanism-linked cellular biomarkers are demonstrated for both LRRK2 and c-Abl.",
        "evidence_basis": "LRRK2/c-Abl potency, pRab10/pCrkL modulation, pS935 preservation and c-Abl co-crystal provide direct functional validation.",
        "main_line_summary": "Strong direct preclinical validation of intended dual-target pharmacology.",
        "why_not_higher": "Score 3 is the maximum; LRRK2 Type II binding itself has not yet been structurally confirmed for the assessed compounds.",
        "investigation_note": "Do not interpret the c-Abl co-crystal as direct structural proof of the LRRK2 binding mode.",
        "uncertain_points": [
          "LRRK2 cryo-EM structural confirmation was planned rather than completed in the reviewed deck."
        ],
        "source_ids": [
          "S3",
          "S14",
          "S15",
          "S16"
        ]
      },
      "platform_attractiveness": {
        "score": 1,
        "evidence_type": "E1_company_claim_or_scientific_rationale_only",
        "evidence_type_reason": "A reusable AI-enabled drug discovery engine is disclosed and used, but quantitative technical superiority is not benchmarked.",
        "evidence_basis": "1ST-501 and the 671-compound 1ST-104 chemistry campaign demonstrate platform use and scalability.",
        "main_line_summary": "Reusable AI/medchem infrastructure is credible, but differentiated technical performance is not quantitatively demonstrated.",
        "why_not_higher": "No platform-vs-comparator quantitative result on hit rate, cycle time, quality or cost.",
        "investigation_note": "Asset potency and PK are not double-counted as evidence of platform superiority.",
        "uncertain_points": [
          "Magnitude of AI contribution versus conventional medicinal chemistry is unknown."
        ],
        "source_ids": [
          "S1",
          "S3"
        ]
      },
      "expansion_potential": {
        "score": 0,
        "evidence_type": "E0_not_found_or_not_assessable",
        "evidence_type_reason": "No additional active indication tied specifically to 1ST-104 was verified.",
        "evidence_basis": "Official pipeline and supplied development plan identify Parkinson's disease only.",
        "main_line_summary": "No confirmed asset-specific indication expansion beyond Parkinson's disease.",
        "why_not_higher": "No additional indication with explicit 1ST-104 biological rationale or program status was verified.",
        "investigation_note": "Other 1STBIO neurodegeneration programs are separate assets and cannot be counted as 1ST-104 expansion.",
        "uncertain_points": [],
        "source_ids": [
          "S1",
          "S3"
        ]
      },
      "data_maturity": {
        "score": 3,
        "evidence_type": "E3_asset_specific_preclinical_or_technical_evidence",
        "evidence_type_reason": "Multiple complementary quantitative domains include target pharmacology, cellular biomarkers, selectivity, ADME and CNS PK.",
        "evidence_basis": "Lead-optimization evidence spans potency, downstream PD, off-target profile, brain penetration and iPSC-derived microglia.",
        "main_line_summary": "Broad and quantitative lead-optimization package across several development questions.",
        "why_not_higher": "Score 3 is the maximum; candidate nomination, completed in vivo efficacy and development-grade toxicology remain outstanding.",
        "investigation_note": "hERG and lung safety require particular follow-up before candidate selection is considered de-risked.",
        "uncertain_points": [
          "No completed 1ST-104 in vivo efficacy package was identified.",
          "FB-4853 repeat-dose lung toxicity results were not present in the reviewed attachment.",
          "FB-4853 hERG IC50 is 8.75 micromolar."
        ],
        "source_ids": [
          "S3"
        ]
      },
      "marketability": {
        "score": 0,
        "evidence_type": "E0_not_found_or_not_assessable",
        "evidence_type_reason": "No reliable asset-specific forecast and insufficient evidence for a non-speculative A/B/C/D calculation.",
        "evidence_basis": "PD population is large, but intended treatable subgroup and an appropriate disease-modifying benchmark price are unresolved.",
        "main_line_summary": "Commercial opportunity may be large, but asset-specific obtainable peak sales cannot yet be reliably quantified.",
        "why_not_higher": "No reliable calculation or asset-specific external forecast supports a sales threshold.",
        "investigation_note": "Broad versus LRRK2-driven patient selection materially changes TAP and competition assumptions after the LUMA result.",
        "uncertain_points": [
          "Commercial target population is not defined.",
          "No appropriate approved disease-modifying benchmark price was identified.",
          "No asset-specific 1ST-104 peak-sales forecast was verified."
        ],
        "source_ids": [
          "S7",
          "S18",
          "S19"
        ],
        "calculation": {
          "commercial_rationale_status": "insufficient_evidence",
          "commercial_rationale_failure_reason": "Treatable subgroup, valid benchmark annualized net price and asset-specific forecast are insufficiently established.",
          "A_targetable_addressable_patient": {
            "targetable_addressable_patient": null,
            "formula": "US TAP = US Patient Pool x Diagnosis Rate x Eligibility Rate x Treatable Subgroup Rate"
          },
          "B_unrisked_peak_sales": {
            "unrisked_peak_sales": null,
            "sales_unit": "million USD",
            "formula": "US Unrisked Peak Sales = US TAP x Benchmark Annualized Net Price x Peak Penetration x Treatment Duration Factor"
          },
          "C_obtainable_peak_sales": {
            "obtainable_peak_sales": null,
            "sales_unit": "million USD",
            "formula": "US Obtainable Peak Sales = US Unrisked Peak Sales x Competition Haircut x Pricing Power Adjustment"
          },
          "D_global_obtainable_peak_sales": {
            "source_geography": "US",
            "global_multiplier": 1.5,
            "global_obtainable_peak_sales": null,
            "sales_unit": "million USD",
            "formula": "Global Obtainable Peak Sales = US Obtainable Peak Sales x 1.5"
          }
        }
      }
    },
    "total_score": 12,
    "max_score": 21
  },
  "validation": {
    "uncertain_points": [
      "Formal 1ST-104 development-candidate nomination has not been verified.",
      "Direct structural confirmation of Type II binding to LRRK2 remains unverified in the supplied evidence.",
      "Completed asset-specific in vivo efficacy and repeat-dose lung toxicity data were not found in the reviewed deck.",
      "FB-4853A hERG IC50 of 8.75 micromolar requires exposure-margin assessment.",
      "Broad versus LRRK2-driven Parkinson's patient-selection strategy is unresolved.",
      "No reliable asset-specific peak-sales forecast is available."
    ],
    "cross_checked_facts": [
      "1ST-104 identity, LRRK2/c-Abl target and Parkinson's indication are supported by the official 1STBIO pipeline and supplied company deck.",
      "Active lead-optimization work is documented in the March 2026 company deck while the current public pipeline remains active.",
      "LRRK2 Parkinson's disease relevance is independently supported by human genetics.",
      "NEU-411 Phase 2 and BIIB122 Phase 2a competitor statuses were checked against ClinicalTrials.gov and sponsor sources."
    ],
    "source_registry": [
      {
        "source_id": "S1",
        "source_title": "1ST Biotherapeutics R&D Pipeline",
        "source_url": "https://www.1stbio.com/rd-pipeline/",
        "source_type": "official_company",
        "verified": true
      },
      {
        "source_id": "S2",
        "source_title": "1ST Biotherapeutics Joins The Michael J. Fox Foundation LITE Program",
        "source_url": "https://www.1stbio.com/wp-content/uploads/2025/04/1st-biotherapeutics-joins-the-michael-j-fox.pdf",
        "source_type": "company_press_release",
        "verified": true
      },
      {
        "source_id": "S3",
        "source_title": "[104] Current Progress and Key Findings_NCDP.pdf",
        "source_url": "",
        "source_type": "user_uploaded_file",
        "verified": true
      },
      {
        "source_id": "S4",
        "source_title": "1STBIO Series D Financing Announcement",
        "source_url": "https://www.1stbio.com/wp-content/uploads/2026/01/1STBIO_Series_D_completed.pdf",
        "source_type": "company_press_release",
        "verified": true
      },
      {
        "source_id": "S5",
        "source_title": "Mutations in LRRK2 cause autosomal-dominant parkinsonism with pleomorphic pathology",
        "source_url": "https://pubmed.ncbi.nlm.nih.gov/15541309/",
        "source_type": "peer_reviewed",
        "verified": true
      },
      {
        "source_id": "S6",
        "source_title": "NEU-411 Phase 2 NEULARK - NCT06680830",
        "source_url": "https://clinicaltrials.gov/study/NCT06680830",
        "source_type": "clinical_registry",
        "verified": true
      },
      {
        "source_id": "S7",
        "source_title": "BIIB122 Phase 2b LUMA Update",
        "source_url": "https://investors.biogen.com/news-releases/news-release-details/biogen-and-denali-therapeutics-provide-update-phase-2b-luma",
        "source_type": "company_press_release",
        "verified": true
      },
      {
        "source_id": "S8",
        "source_title": "Brenig Therapeutics BT-267 Pipeline",
        "source_url": "https://www.brenigther.com/",
        "source_type": "official_company",
        "verified": true
      },
      {
        "source_id": "S9",
        "source_title": "Arvinas ARV-102 Phase 1 Parkinson's Disease Data",
        "source_url": "https://ir.arvinas.com/news-releases/news-release-details/arvinas-announces-positive-phase-1-data-arv-102-showing-greater/",
        "source_type": "company_press_release",
        "verified": true
      },
      {
        "source_id": "S10",
        "source_title": "BIIB094 REASON Phase 1 - NCT03976349",
        "source_url": "https://clinicaltrials.gov/study/NCT03976349",
        "source_type": "clinical_registry",
        "verified": true
      },
      {
        "source_id": "S11",
        "source_title": "LRRK2-targeting antisense oligonucleotide in Parkinson's disease",
        "source_url": "https://www.nature.com/articles/s41591-026-04262-4",
        "source_type": "peer_reviewed",
        "verified": true
      },
      {
        "source_id": "S12",
        "source_title": "FinsnoBio LRRK2 Type II Pipeline",
        "source_url": "https://www.finsnobio.com/pipeline",
        "source_type": "official_company",
        "verified": true
      },
      {
        "source_id": "S14",
        "source_title": "c-Abl phosphorylates alpha-synuclein and regulates its degradation",
        "source_url": "https://pubmed.ncbi.nlm.nih.gov/24412932/",
        "source_type": "peer_reviewed",
        "verified": true
      },
      {
        "source_id": "S15",
        "source_title": "Phos-tag analysis of Rab10 phosphorylation by LRRK2",
        "source_url": "https://pubmed.ncbi.nlm.nih.gov/27474410/",
        "source_type": "peer_reviewed",
        "verified": true
      },
      {
        "source_id": "S16",
        "source_title": "Type II kinase inhibitors that target Parkinson's disease-associated LRRK2",
        "source_url": "https://pubmed.ncbi.nlm.nih.gov/40465731/",
        "source_type": "peer_reviewed",
        "verified": true
      },
      {
        "source_id": "S17",
        "source_title": "Evaluation of c-Abl inhibitor vodobatinib in early Parkinson's disease",
        "source_url": "https://www.nature.com/articles/s41531-026-01275-1",
        "source_type": "peer_reviewed",
        "verified": true
      },
      {
        "source_id": "S18",
        "source_title": "Parkinson's Foundation Prevalence and Incidence",
        "source_url": "https://www.parkinson.org/understanding-parkinsons/statistics/prevalence-incidence",
        "source_type": "patient_foundation",
        "verified": true
      },
      {
        "source_id": "S19",
        "source_title": "BIIB122 Phase 2a BEACON - NCT06602193",
        "source_url": "https://clinicaltrials.gov/study/NCT06602193",
        "source_type": "clinical_registry",
        "verified": true
      }
    ]
  },
  "final_insight": {
    "one_line_summary": "1ST-104 combines strong PD target relevance and broad lead-optimization data with a differentiated Type II-like profile, but candidate, efficacy, safety and commercial de-risking remain incomplete in an increasingly challenging LRRK2 field.",
    "recommendation": "Watch",
    "most_important_diligence_question": "Can the nominated 1ST-104 candidate demonstrate in vivo efficacy and durable CNS target coverage with adequate hERG and lung safety margins, and what biomarker-defined Parkinson's population will be targeted after the BIIB122 LUMA and c-Abl Phase 2 failures?"
  },
  "company_profile": {
    "headquarters": "Yongin-si, Gyeonggi-do, Republic of Korea",
    "company_stage": "Private, clinical-stage biopharmaceutical company",
    "platform_summary": "1ST-501 AI Drug Discovery Engine; 1ST-104 used an AI-driven in-house medicinal chemistry workflow, but quantitative platform superiority versus a comparator is not established."
  },
  "competitive_analysis": {
    "competitive_density": "High",
    "similarity_summary": {
      "similar_pipeline_count": 7,
      "high_similarity_count": 4,
      "medium_similarity_count": 3,
      "low_similarity_count": 0
    },
    "competitor_table": [
      {
        "competitor_asset": "NEU-411",
        "company": "Neuron23",
        "modality": "Small molecule",
        "target_or_moa": "Selective LRRK2 kinase inhibitor",
        "stage": "Phase 2",
        "similarity_level": "High",
        "why_it_matters": "Direct oral LRRK2 competitor in genetically selected LRRK2-driven early Parkinson's disease.",
        "source_url": "https://clinicaltrials.gov/study/NCT06680830"
      },
      {
        "competitor_asset": "BIIB122 / DNL151",
        "company": "Denali Therapeutics / Biogen",
        "modality": "Small molecule",
        "target_or_moa": "LRRK2 kinase inhibitor",
        "stage": "Phase 2a BEACON active; idiopathic PD development discontinued after Phase 2b LUMA",
        "similarity_level": "High",
        "why_it_matters": "Direct target/modality benchmark and major negative efficacy precedent in broad early PD.",
        "source_url": "https://investors.biogen.com/news-releases/news-release-details/biogen-and-denali-therapeutics-provide-update-phase-2b-luma"
      },
      {
        "competitor_asset": "BT-267",
        "company": "Brenig Therapeutics",
        "modality": "Small molecule",
        "target_or_moa": "Brain-penetrant selective LRRK2 inhibitor",
        "stage": "Phase 1",
        "similarity_level": "High",
        "why_it_matters": "Direct oral CNS LRRK2 competitor emphasizing selectivity and brain exposure.",
        "source_url": "https://www.brenigther.com/"
      },
      {
        "competitor_asset": "Type II LRRK2 program",
        "company": "FinsnoBio",
        "modality": "Small molecule",
        "target_or_moa": "Type II and G2019S-focused LRRK2 inhibition",
        "stage": "Preclinical / IND-directed; exact asset stage undisclosed",
        "similarity_level": "High",
        "why_it_matters": "Directly competes with the Type II LRRK2 differentiation concept.",
        "source_url": "https://www.finsnobio.com/pipeline"
      },
      {
        "competitor_asset": "ARV-102",
        "company": "Arvinas",
        "modality": "PROTAC",
        "target_or_moa": "LRRK2 protein degradation",
        "stage": "Phase 1 PD data",
        "similarity_level": "Medium",
        "why_it_matters": "Alternative LRRK2 modality with demonstrated human CNS target modulation.",
        "source_url": "https://ir.arvinas.com/news-releases/news-release-details/arvinas-announces-positive-phase-1-data-arv-102-showing-greater/"
      },
      {
        "competitor_asset": "BIIB094",
        "company": "Ionis / Biogen",
        "modality": "RNA therapy",
        "target_or_moa": "LRRK2-targeting antisense oligonucleotide",
        "stage": "Phase 1 completed",
        "similarity_level": "Medium",
        "why_it_matters": "Clinical human evidence for direct LRRK2 lowering through a distinct modality.",
        "source_url": "https://www.nature.com/articles/s41591-026-04262-4"
      },
      {
        "competitor_asset": "Vodobatinib",
        "company": "SPARC",
        "modality": "Small molecule",
        "target_or_moa": "c-Abl kinase inhibitor",
        "stage": "Phase 2 completed; negative efficacy",
        "similarity_level": "Medium",
        "why_it_matters": "Clinical negative precedent for the c-Abl component of 1ST-104's dual mechanism.",
        "source_url": "https://www.nature.com/articles/s41531-026-01275-1"
      }
    ],
    "similar_pipelines": [
      {
        "company": "Neuron23",
        "asset_name": "NEU-411",
        "similarity_score": 3,
        "matched_dimensions": [
          "Parkinson's disease",
          "LRRK2",
          "Small molecule"
        ],
        "shared_data_points": [
          "Brain penetration",
          "LRRK2 pathway inhibition"
        ]
      },
      {
        "company": "Denali Therapeutics / Biogen",
        "asset_name": "BIIB122 / DNL151",
        "similarity_score": 3,
        "matched_dimensions": [
          "Parkinson's disease",
          "LRRK2",
          "Small molecule"
        ],
        "shared_data_points": [
          "pS935 biomarker",
          "pRab10 biomarker",
          "Oral CNS-directed pharmacology"
        ]
      },
      {
        "company": "Brenig Therapeutics",
        "asset_name": "BT-267",
        "similarity_score": 3,
        "matched_dimensions": [
          "Parkinson's disease",
          "LRRK2",
          "Small molecule"
        ],
        "shared_data_points": [
          "Brain penetration",
          "Kinase selectivity"
        ]
      },
      {
        "company": "FinsnoBio",
        "asset_name": "Type II LRRK2 program",
        "similarity_score": 3,
        "matched_dimensions": [
          "Parkinson's disease",
          "LRRK2",
          "Small molecule",
          "Type II pharmacology"
        ],
        "shared_data_points": [
          "Type II binding concept",
          "Potential lung-safety differentiation"
        ]
      },
      {
        "company": "Arvinas",
        "asset_name": "ARV-102",
        "similarity_score": 2,
        "matched_dimensions": [
          "Parkinson's disease",
          "LRRK2 pathway"
        ],
        "shared_data_points": [
          "CNS target modulation",
          "Neuroinflammatory biomarkers"
        ]
      },
      {
        "company": "Ionis / Biogen",
        "asset_name": "BIIB094",
        "similarity_score": 2,
        "matched_dimensions": [
          "Parkinson's disease",
          "LRRK2 pathway"
        ],
        "shared_data_points": [
          "Direct LRRK2 modulation"
        ]
      },
      {
        "company": "SPARC",
        "asset_name": "Vodobatinib",
        "similarity_score": 2,
        "matched_dimensions": [
          "Parkinson's disease",
          "c-Abl pathway"
        ],
        "shared_data_points": [
          "Brain-penetrant small molecule",
          "c-Abl inhibition"
        ]
      }
    ]
  }
}
```
