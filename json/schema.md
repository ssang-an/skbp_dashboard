...
- `meta.pipeline_metadata`: internal, non-scientific Listing metadata carried from Tab0 into the matching Fast Triage and Full Scout records. It is `{ listed_at, comment, contact, updated_at }`; `contact` is free text (name, email, phone, organization, or role). Listing records that are not yet researched retain the same fields in `json/candidate-queue.json` until promotion. Blank spreadsheet cells never erase an existing Comment/Contact; an explicit Tab0 edit may clear a field. These values are displayed in Fast Triage Quick Summary and Full Scout Team Review, are searchable/exportable from Tab0, and must never be copied into GPT source Markdown, evidence, scoring, or rubric decisions.
- `json/candidate-queue.json.listing_details`: Tab0-only optional retrieval context for a still-pending Listing entry: `{ country, modality, target, main_indication, stage }`. It is shown and exported in the Tab0 progress table and may be appended to the Fast Triage prompt only as user-provided identification context. It is independently verified by research and is deliberately not promoted into `pipeline-records.json`, so it cannot overwrite official Fast Triage/Full Scout fields, evidence, scoring, or rubric decisions. Once an identity has a researched record, Tab0 display uses official fields in precedence order `Full Scout → Fast Triage → Listing`; field-level blanks in Full Scout retain a non-blank Fast Triage value. This is a read-only display replacement, not a write-back to the Listing entry.
