...
- `meta.pipeline_metadata`: internal, non-scientific Listing metadata carried from Tab0 into matching Fast Triage and Full Scout records. It is `{ listed_at, comment, comment_author, comment_source, comment_created_at, comment_updated_at, comment_entries, listing_details, contact, contact_author, contact_source, contact_created_at, contact_updated_at, website, asset_aliases, company_aliases, updated_at }`. `comment` remains the compatibility aggregate, while `comment_entries` preserves each distinct Listing import/direct-post event with its timestamp and source for the Tab0 Comment popover; an exactly repeated comment is not added again. `listing_details` is the latest exact Asset+Company Listing display overlay for Tab0 (Location, Modality, Target, Main indication, Pipeline Stage, and Website): incoming non-missing Listing values take priority and blank/Unknown/N/A values fall back to the prior Listing or official value. It never overwrites Fast Triage/Full Scout scientific fields. `website` holds one valid `http(s)` URL for the table external-link control; it is a user-provided navigation hint, not research evidence. When a reviewer connects two pending Listing rows, the selected existing/new Listing value is used for every non-empty Listing field (Asset, Company, Location, Modality, Target, Main indication, Pipeline Stage, and Website); blank selected fields are filled from the other row. The non-selected Website is not retained. Comment and Contact accumulate, while Asset/Company aliases are retained for search and later identity matching. Listing metadata is operational context only: it is shown in Tab0 and the Fast Triage/Full Scout operational views but never copied into GPT source Markdown, evidence, scoring, or rubric decisions.
...
- `json/candidate-queue.json.listing_details`: Tab0-only optional retrieval context for a still-pending Listing entry: `{ country, modality, target, main_indication, stage, website }`. It is shown and exported in the Tab0 progress table and may be appended to the Fast Triage prompt only as user-provided identification context. Website accepts one `http(s)` URL and renders as an external-link pill. Administrators may correct the pending Listing row inline; `manual_fields` retains the corrected field, editor, and timestamp solely to provide a subtle manual-edit cue. Excel headers are matched by meaning (for example `HQ Location → Country`); repeated operational-note headers such as `Comment`, `Priority`, `Reason for Priority`, and `Next Step` concatenate into Comment, while `Meeting History`/`Contact History` concatenate into Contact unless their combined cell begins with `X` and includes a note, in which case the note appends to Comment and Contact is cleared. When Asset and Company both match an existing Listing identity exactly, non-missing incoming Listing values are the display representative and prior values fill only its blanks/Unknown/N/A markers. Other automatic matches retain the richer-row merge behavior. Blank text and the canonical absence markers `Unknown`, `-`, `N/A`, and `Not Available` are all missing values for Listing merge purposes. Descriptive same-company names become a review candidate only when at least two meaningful program terms match after normalization; generic construction words such as `small molecule` and `inhibit` do not create a match on their own. In a review modal, direct reciprocal alias pairs are one decision group: connecting either row applies the matching connection to its reverse pair. For a reviewed Listing-to-Listing merge, new incoming values are the default representative; every selected-row Listing value is retained unless it is a missing value, in which case the other row fills it. Asset/Company aliases plus non-empty Comments and Contacts are preserved. Pending Listing data is independently verified by research. For a researched exact-identity re-import, only the Tab0 display overlay is retained in `pipeline_metadata`; it cannot overwrite official Fast Triage/Full Scout fields, evidence, scoring, or rubric decisions. While an entry is pending, Tab0 displays these Listing values. Once an identity has a researched record, Tab0 uses the exact-identity `listing_details` display overlay first and then Full Scout → Fast Triage official fields as fallbacks. This is a read-only display replacement, not a write-back to the scientific report; edits must then be made in Tab1 or Tab2.
...
- `meta.human_review.overrides.total_score`: optional TAB2 Full Scout Total Score correction (0-21). It is independent from the seven criterion overrides, is audit-logged with reviewer ID, and receives the same refined red human-edit treatment. TAB3 displays this Tab2 score as a read-only circular badge labeled **Scout Score**; legacy `meta.focus_management.total_score_override` values are retained in old records for compatibility but are no longer displayed or editable.
- **Multi-Project Shortlisting**: TAB3 supports team-defined Shortlisting Projects beyond the built-in OIC sheet. `json/shortlisting-projects.json` (flat list, same `write_json_atomic`/lazy-seed convention as `json/candidate-queue.json`) holds Project definitions: `{id, name, description, is_default, archived, metric_columns: [...], classification_columns: [...], created_by_name/created_by_user_id/created_by_email, created_at, updated_at}`. `id` is the fixed literal `oic_default` for the seeded default Project, or `proj_<uuid hex10>` for teams created via `POST /api/shortlisting/projects` (any logged-in user, not admin-gated). Each `metric_columns` entry is `{id, label, description, label_en, description_en, return_type: boolean|list|number|date|text, is_builtin, options, max_value, created_by_name, created_at}` — `id` is `builtin_*` for the OIC default Project's 7 legacy columns (descriptive metadata only; their rendering/logic stays hardcoded and unrelated to this table) or `metric_<uuid hex10>` for custom columns added via `POST /api/shortlisting/projects/{project_id}/columns` (`list` requires 1-20 `options`; `number` accepts an optional integer `max_value` 1-1,000,000 — when set, values are bounded to 0..max_value, otherwise any finite number is accepted; `date` needs no extra config; capped at 20 custom columns per Project). Custom columns are managed from the Shortlisting table's "지표 관리" panel, which lists every non-builtin column for the active Project (지표/설명/입력 columns) with an inline add row and a per-row delete button (`DELETE /api/shortlisting/projects/{project_id}/columns/{column_id}`, rejects `builtin_*` ids). Each `classification_columns` entry is `{id, label, description, label_en, description_en, created_by_name, created_at}` (`id` prefixed `class_<uuid hex10>`) — a free-text description of one possible final-decision bucket a Project's owner defines by combining its metrics (conceptually parallel to the OIC default Project's hardcoded 투자/Value Up/공동연구 partnership classification, but purely descriptive text — there is no automatic classification logic for custom Projects). Managed only for non-`oic_default` Projects (owner-only) via `POST`/`DELETE /api/shortlisting/projects/{project_id}/classifications[/{classification_id}]`, capped at 12 per Project, surfaced in the Project Settings modal's "분류 관리" tab (지표 관리 옆) as an ordered table (순번/분류/설명) with an inline add row, and rendered read-only inside the 판단근거 (criteria) drawer for that Project — see the Shortlisting Project permissions bullet below for how the drawer picks OIC's hardcoded content vs. a custom Project's own `classification_columns`/`metric_columns`. `label_en`/`description_en` on both `metric_columns` and `classification_columns` are a best-effort literal OpenRouter KO->EN translation attempted once at creation time via `call_openrouter_translate_ko_to_en()` — empty when `OPENROUTER_API_KEY` is unset or the call fails, in which case the 판단근거 drawer's English toggle falls back to the Korean text. Items created before this translation step existed (or whose first attempt failed) can be backfilled on demand via `POST /api/shortlisting/projects/{project_id}/columns/{column_id}/translate` or `POST /api/shortlisting/projects/{project_id}/classifications/{classification_id}/translate` (owner-only, re-runs the same translation and overwrites `label_en`/`description_en` in place) — surfaced as a "번역"/"재번역" button per row in both the 지표 관리 and 분류 관리 tables. A record's per-Project tracking/values live in a **new, separate namespace, `meta.shortlisting_projects.<project_id>`** — structurally parallel to `meta.focus_management` but never used for `oic_default`, which keeps using `meta.focus_management` unchanged for zero-migration compatibility. Shape: `{is_tracked, tracking_status, added_at, removed_at, custom_score (0-9), metric_values: {<metric_column_id>: value}, classification (a classification_columns id, or "" when unset), due_date, owner_name/owner_user_id/owner_email, action_plan, user_comment, updated_at/updated_source/updated_by}`. `classification` is a manual per-record pick from the Project's own `classification_columns` (`field: "classification"` on the same `PATCH`, 404s on an unrecognized id) — there is no automatic classification logic for custom Projects (unlike OIC's rule-based `partnership_type`), so this is simply "which of this Project's defined buckets does a reviewer judge this pipeline to fall into." The Shortlisting table's Custom Review column group renders it as a colored dropdown pill (`.partnership-edit-select`, cycling the same four OIC tone colors by option index) immediately before the Project's `metric_columns`, visually matching (but functionally independent of) OIC's Filter 3 column, mutated via `PATCH /api/records/{record_id}/shortlisting-projects/{project_id}` (login-only, mirrors `/focus-management`'s `add`/`stationary`/`remove`/`update` actions and validation, but is rejected for `oic_default`). **Total Score** (the Shortlisting table column immediately left of Filter 2) is `Scout Score (Full Scout 0-21) + Custom Score (0-9) = 0-30`, computed identically for every Project by `shortlisting_total_score()` in `main.py` and mirrored client-side in `flattenRecord()`/`recomputeShortlistingScoreFields()`; it is not a stored/cached field. Records tracked into one Project are independent of every other Project's tracked set — the OIC "즐겨찾기" star control in Full Scout targets whichever Project is currently active in the Shortlisting Project selector. Each Project's `due_date` (Action date) is likewise fully independent per Project, stored at `meta.shortlisting_projects.<project_id>.due_date` for custom Projects (parallel to `meta.focus_management.due_date` for `oic_default`); the same asset can carry a different Action Date, owner, and reminder history in each Project it is tracked in.
- **Shortlisting Project permissions**: every Project object — including `oic_default` — carries `members: [{email, role, added_by, added_at}]` (`role` is `"owner"` or `"write"`; an email absent from `members` is implicitly read-only). `oic_default` starts with an empty `members: []` (it has no `created_by_email` to seed from), so only site admins act as owner on it until one explicitly grants owner/write to specific emails via its own Project Settings gear — the same mechanism as any custom Project. `load_shortlisting_projects()` lazily backfills `members` on any pre-existing Project (including `oic_default`) that predates this field, seeding a custom Project's `created_by_email` as its sole owner (same lazy-seed convention as the default-Project insert; `oic_default` gets an empty list). `shortlisting_project_role_for_account()` resolves a user's effective role (`owner`/`write`/`read`) identically for every Project; site admins (`is_auth_admin`) are always treated as `owner` on every Project without needing a `members` entry. This role gates: renaming/archiving a Project (`PATCH`/`DELETE /api/shortlisting/projects/{project_id}`, owner-only), adding/deleting custom metric columns (`POST`/`DELETE .../columns[...]`, owner-only — **and unconditionally blocked for `oic_default` regardless of role**, since its 7 columns are hardcoded and not yet meant to be extended), adding/deleting classification rows (`POST`/`DELETE .../classifications[...]`, same owner-only + `oic_default`-blocked rule), every per-record Project mutation — tracking/untracking and all field edits including `due_date` — via `PATCH /api/records/{record_id}/shortlisting-projects/{project_id}` for custom Projects (write-or-above, previously any logged-in user), and now also `PATCH /api/records/{record_id}/focus-management` for `oic_default` (write-or-above via the same `members`/admin-bypass check, replacing the old `is_admin`-only gate). `GET /api/shortlisting/projects` remains fully public/read-open and additionally serializes an ephemeral, never-persisted `current_user_role` on each Project (including `oic_default`) so the frontend can gate UI without a second round-trip. Membership itself is owner-managed via `POST /api/shortlisting/projects/{project_id}/members` (upsert an email's role) and `DELETE .../members/{email}` — now allowed for `oic_default` too; both reject leaving a Project with zero owners (removing or demoting the last remaining owner returns 400). The Shortlisting table's settings gear icon (shown next to any Project — including OIC — where the current user is owner) opens a two-tab Project Settings modal — 멤버 관리 (members) and 지표 관리 (metric columns; for `oic_default` this tab lists its 7 builtin columns read-only with the add-form replaced by a locked-note, since column management is blocked there). The former standalone "지표 관리" toolbar button was removed; this modal is the only entry point. The Action Date reminder scheduler (`run_action_date_reminders`) scans both `meta.focus_management` and every `meta.shortlisting_projects.<project_id>` bucket independently, resolving each Project's own owner/recipient via `action_date_owner_account()` and tracking a separate `action_date_reminders` dedup history per Project. The 멤버 관리 tab's "구성원 추가" picker sources candidates from `GET /api/users/directory` (login-only, not admin-gated; returns only `{name, email}` for active accounts, excluding role/session/activity fields), supports a name+email search filter above the checkbox list, and lets an owner check several accounts at once, applying one selected role to all of them via repeated calls to the single-member `POST` endpoint above. The existing-members table also shows each member's directory name alongside their email. The OIC default Project's builtin "DD" (Due Diligence) column is temporarily hidden from the Custom Review table (removed from `FOCUS_TABLE_OIC_METRIC_COLUMN_KEYS` in `src/app.js`), without touching the underlying `ddStatus` data or its Full Scout filter — only the visible column and its header/colspan were removed.
...
- `meta.topic_notes`: array of inline Team Review notes that remain separate from `source_report.raw_markdown`. Each note is `{id, topic_id, topic_key, topic_title, body, author_id, author_name, created_at, updated_at}`. `topic_id` maps to a rendered report heading; `topic_key` removes section numbering and normalizes the heading so a note can remap after a same-asset Full Scout reupload. On an overwritten report, a note whose heading cannot be mapped is moved into Team Review Comments with its author/provenance and a `GPT 원문 재업로드 · 매핑되지 않은 Topic 메모` label, rather than being discarded or left hidden. Matching notes remain attached to their headings. Confirmed reupload preserves this operational context through `preserve_dashboard_meta`; note create/update/delete operations are authenticated and audit-logged.
- `meta.report_reupload_history`: up to 10 recoverable pre-reupload revisions. A same-asset source-report overwrite stores the previous source report and a deep record snapshot before replacement; confirmed same-pipeline reuploads require an administrator, and the snapshot plus visible edit history retain that administrator's name (falling back to the authenticated email only when no display name is configured). Nested reupload history is removed from each snapshot to prevent recursive growth; Topic notes and operational metadata continue independently on the live record.
