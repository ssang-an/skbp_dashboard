...
- `meta.pipeline_metadata`: internal, non-scientific Listing metadata carried from Tab0 into the matching Fast Triage and Full Scout records. It is `{ listed_at, comment, comment_author, comment_source, comment_created_at, comment_updated_at, contact, website, updated_at }`; `contact` is free text (name, email, phone, organization, role, contact date, or `O`). `website` retains only the first valid `http(s)` URL from the Listing spreadsheet cell and is a user-provided navigation hint, not research evidence. Exact `X` and hyphen-only (`-`, `–`, `—`) Contact values explicitly mean no contact history and clear any prior Contact value; a blank cell means no history only when no prior Contact exists, so a blank spreadsheet cell does not erase an existing note. Tab0 shows a non-clickable `-` for no-history state. Listing records that are not yet researched retain the same fields in `json/candidate-queue.json` until promotion. Bulk Listing imports save Comment as `Tab 0 Team Review`; an administrator-written Listing Comment Post, or a later administrator edit of an imported Comment, replaces that Listing Comment and records the administrator name and timestamp. Blank spreadsheet cells never erase an existing Comment/Contact/Website; an explicit Tab0 edit may clear a field. These values are displayed in Fast Triage Quick Summary and Full Scout Team Review, are searchable/exportable from Tab0, and must never be copied into GPT source Markdown, evidence, scoring, or rubric decisions. Once a researched record exists, the Listing Comment is also represented as a durable source-attributed operational comment. Website is shared across the identity group, is registered or edited only in Tab0 Listing, and appears in Tab1/Tab2 as an external-link control that opens the shared URL. Tab0's Comment popover is a derived operational feed: it shows the Listing Comment plus human-authored Tab1/Tab2 Final Comments, Fast Triage criterion notes, and direct Team Review comments in timestamp order. AI-generated qualitative answers and imported duplicates remain excluded.
- `meta.collaboration`: Team Review Workspace comment thread. Normal comments retain their author and reply relation. System-imported cross-workflow comments have `{system_import: true, import_key, source}`: Fast Triage Final Comment and Fast Triage criterion notes are copied once to matching Full Scout records with source-labelled authors, while Listing comments retain the saved Listing author (`Tab 0 Team Review` for bulk imports, or the administrator for a direct Listing Comment Post) on researched records. `deleted_import_keys` prevents an administrator-deleted imported comment from returning on a later reupload. These comments are operational review context only and never enter GPT source Markdown, evidence, scoring, or rubric decisions.
