...
- `meta.pipeline_metadata`: internal, non-scientific Listing metadata carried from Tab0 into the matching Fast Triage and Full Scout records. It is `{ listed_at, comment, comment_author, comment_source, comment_created_at, comment_updated_at, contact, contact_author, contact_source, contact_created_at, contact_updated_at, website, asset_aliases, company_aliases, updated_at }`; `contact` is free text (name, email, phone, organization, role, contact date, or `O`). Any non-empty Excel Contact value is Contact History even without an explicit `O`; non-duplicate values accumulate, while a direct Tab0 Contact edit corrects/replaces only that Listing Contact value. Exact `X` and hyphen-only (`-`, `–`, `—`) alone mean no Listing Contact information and clear the prior Listing Contact value. An `X` marker followed by a note (for example `X: legal review pending`) moves the note into the Listing Comment instead and creates no Contact History post; it does not delete existing human-authored Contact History or Comment posts. A blank spreadsheet cell does not erase an existing note. `website` retains only the first valid `http(s)` URL from the Listing spreadsheet cell and is a user-provided navigation hint, not research evidence. When a Tab0 Excel row matches an existing Fast Triage or Full Scout Pipeline, its raw Asset and Company inputs are automatically accumulated in `asset_aliases` and `company_aliases`; they are search and later identity-matching metadata only, while the Pipeline Table continues to display the official report's main Asset and Company names. Tab0 shows a non-clickable `-` for no-history state. Listing records that are not yet researched retain the same fields in `json/candidate-queue.json` until promotion. Bulk Listing imports save Comment as `Team Review` and display `일괄 업로드: Tab 0 · Comment · Team`; direct Comment and Contact History entries record their respective administrator, source, and timestamp, and can subsequently be edited or deleted only by that administrator. Blank spreadsheet cells never erase an existing Comment/Contact/Website; an explicit Tab0 edit may clear a field. These values are displayed in Fast Triage Quick Summary and Full Scout Team Review, are searchable/exportable from Tab0, and must never be copied into GPT source Markdown, evidence, scoring, or rubric decisions. Once a researched record exists, Listing Comment and Contact are represented as durable, source-attributed operational posts in one canonical Workspace: Full Scout (Tab2) first, otherwise Fast Triage (Tab1). Human-authored Tab1 Contact History posts are copied into the canonical Full Scout Contact History with their original author and a source breadcrumb; canonical Tab2 posts are not copied back to Tab1. Website is shared across the identity group, is registered or edited only in Tab0 Listing, and appears in Tab1/Tab2 as an external-link control that opens the shared URL. Tab0's Comment and Contact popovers are derived operational feeds: Contact-labelled human-authored Tab1/Tab2 entries appear only in Contact, while the remaining human-authored Final Comments, Fast Triage criterion notes, and direct Team Review comments appear in Comment. AI-generated qualitative answers and imported duplicates remain excluded.
