...
function renderCollaborationPanel(record) {
  ...
  if (elements.detailOiPartnershipOrigin) {
    elements.detailOiPartnershipOrigin.textContent = `OI Partnership v${focus.partnership_classification_criteria_version || '1.3'}`;
...
async function refreshOiPartnership() {
  ...
  try {
    ...
    renderCollaborationPanel(currentRecord);
    setCollaborationStatus(
      `OI Partnership v${data.oi_partnership_criteria_version || '1.3'} 기준 갱신 완료`,
      'success'
    );
  } catch (error) {
    setCollaborationStatus(error.message, 'error');
