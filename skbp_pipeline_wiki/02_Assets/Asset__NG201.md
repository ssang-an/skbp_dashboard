---
type: "asset"
canonical_id: "asset::ng201"
title: "NG201"
aliases:
tags:
  - "pipeline/asset"
  - "skbp/pipeline_finder"
  - "theme/unknown"
created_at: "2026-08-23"
updated_at: "2026-08-23"
source_report: "[[01_Raw_Reports/Neuracle_Genetics_(now_Elisigen)_NG201]]"
source_json: "json/pipeline-records.json"
status: "active"
confidence: "medium"
company: "[[03_Companies/Company__Neuracle_Genetics_(now_Elisigen)|Neuracle Genetics (now Elisigen)]]"
target: "[[04_Targets/Target__Unknown|Unknown]]"
moa: "[[05_MoA/MoA__Unknown|Unknown]]"
modality: "[[06_Modalities_Platforms/Modality__Gene_therapy|Gene therapy]]"
indications:
  - "[[07_Indications/Indication__Neuropathic_pain|Neuropathic pain]]"
scorecard: "[[10_Scorecards/Scorecard__NG201__Neuracle_Genetics_(now_Elisigen)_NG201|Scorecard__NG201__Neuracle_Genetics_(now_Elisigen)_NG201]]"
theme: "[[11_Themes_Clusters/Theme__Unknown|Unknown]]"
cluster: "[[11_Themes_Clusters/Cluster__Unknown|Unknown]]"
total_score: 2
max_score: 9
recommendation: "Do not run Full Scout"
evidence_level: "E0"
---

# NG201

## 1. Snapshot

| Field | Value |
|---|---|
| Company | [[03_Companies/Company__Neuracle_Genetics_(now_Elisigen)|Neuracle Genetics (now Elisigen)]] |
| Asset | NG201 |
| Target | [[04_Targets/Target__Unknown|Unknown]] |
| MoA | [[05_MoA/MoA__Unknown|Unknown]] |
| Modality | [[06_Modalities_Platforms/Modality__Gene_therapy|Gene therapy]] |
| Indication | [[07_Indications/Indication__Neuropathic_pain|Neuropathic pain]] |
| Stage | Preclinical unspecified |
| Theme / Cluster | [[11_Themes_Clusters/Theme__Unknown|Unknown]] / [[11_Themes_Clusters/Cluster__Unknown|Unknown]] |
| Recommendation | Do not run Full Scout |
| Total Score | 2/21 |

## 2. One-line Summary

현재 code continuity 미확인으로 REJECT입니다.

## 3. Why This Asset Matters

- Strategic relevance: TR 2점: NG201은 Neuracle Genetics의 neuropathic pain preclinical asset로 확인되지만 현재 Elisigen의 EG201과 동일 자산인지 검증되지 않았습니다.
- Scientific rationale: MOA 0점: NG201 자체의 target/MoA를 현재 자료에서 확인하지 못했습니다.
- BD relevance: NG201과 EG201이 동일 vector/transgene/construct라는 공식 evidence가 있는가?

## 4. Scorecard

![[10_Scorecards/Scorecard__NG201__Neuracle_Genetics_(now_Elisigen)_NG201]]

## 5. Knowledge Links

### Core Biology
- Target: [[04_Targets/Target__Unknown|Unknown]]
- MoA: [[05_MoA/MoA__Unknown|Unknown]]
- Theme: [[11_Themes_Clusters/Theme__Unknown|Unknown]]
- Cluster: [[11_Themes_Clusters/Cluster__Unknown|Unknown]]

### Development Context
- Company: [[03_Companies/Company__Neuracle_Genetics_(now_Elisigen)|Neuracle Genetics (now Elisigen)]]
- Modality: [[06_Modalities_Platforms/Modality__Gene_therapy|Gene therapy]]
- Indication: [[07_Indications/Indication__Neuropathic_pain|Neuropathic pain]]

### Evidence Sources
- [[09_Evidence_Sources/Source__Neuracle_Genetics_NG201_partnering_profile|Neuracle Genetics NG201 partnering profile]]
- [[09_Evidence_Sources/Source__Elisigen_current_pipeline|Elisigen current pipeline]]
- [[09_Evidence_Sources/Source__Neuracle_Genetics_to_Elisigen_rename_report|Neuracle Genetics to Elisigen rename report]]

## 6. Key Diligence Questions

- NG201과 EG201이 동일 vector/transgene/construct라는 공식 evidence가 있는가?

## 7. Evidence Gaps

- NG201 vs EG201 relationship unresolved

## 8. Score History

| Date | Total Score | Recommendation | Scorecard |
|---|---:|---|---|
| 20260820 | 2/21 | Do not run Full Scout | [[10_Scorecards/Scorecard__NG201__Neuracle_Genetics_(now_Elisigen)_NG201|Scorecard__NG201__Neuracle_Genetics_(now_Elisigen)_NG201]] |

## 9. Raw Report

Source report: [[01_Raw_Reports/Neuracle_Genetics_(now_Elisigen)_NG201|Neuracle_Genetics_(now_Elisigen)_NG201]]
