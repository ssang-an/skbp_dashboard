...
class DashboardInformationArchitectureTests(unittest.TestCase):
    ...
    def test_header_data_upload_shortcut_uses_current_tab_upload_panel(self):
        ...
        self.assertIn("updateHeaderRecordCount()", workflow)
        header_count = function_body(JS, "currentTabRecordCount")
        self.assertIn("state.step0Rows.length", header_count)
        self.assertIn("state.rows.filter(rowMatchesActiveTableMode).length", header_count)
    ...
    def test_step0_reentry_reuses_cached_progress_without_a_second_unchanged_graph_render(self):
        activate = function_body(JS, "activateStep0Panel")
        load = function_body(JS, "loadStep0Progress")

        self.assertIn('if (state.step0Loaded)', activate)
        self.assertIn('renderStep0ProgressTable();', activate)
        self.assertIn('loadStep0Progress({ renderOnlyWhenChanged: true });', activate)
        self.assertIn('function step0ProgressSnapshot(rows, stats, recentStats)', JS)
        self.assertIn('state.step0ProgressLoadRequestId', JS)
        self.assertIn('if (requestId !== state.step0ProgressLoadRequestId) return;', JS)
        self.assertIn('if (renderOnlyWhenChanged && !hasChanged) return;', JS)
        self.assertIn('if (renderOnlyWhenChanged && state.step0Loaded) return;', JS)
    ...
    def test_step0_workflow_map_uses_filtered_rows_and_g6_stage_nodes(self):
        ...
        self.assertIn('full_scout: { color: \'#1baf7a\', size: 13 }', JS)
        self.assertIn('shortlisting: { color: \'#b7791f\', size: 22 }', JS)
        self.assertIn('step0WorkflowMapFor(stage.key)', workflow_map)
        self.assertIn('const STEP0_WORKFLOW_STAGE_STAGGER_MS = 420', JS)
        self.assertIn('const enterDelay = stage.index * STEP0_WORKFLOW_STAGE_STAGGER_MS', workflow_map)
        self.assertIn('--step0-workflow-enter-delay:${enterDelay}ms', workflow_map)
        self.assertIn('Math.floor(bounds.width)', workflow_map)
        self.assertIn('Math.floor(bounds.height)', workflow_map)
        self.assertIn('pending: { color:', JS)
        self.assertIn('shortlisting: { color:', JS)
        self.assertIn('data: { nodes, edges: [] }', workflow_map)
        self.assertIn('step0WorkflowIrregularPosition(rowIndex, stageRows.length, width, height, nodeSize, id)', workflow_map)
        self.assertIn('step0WorkflowSeededUnit', JS)
        self.assertIn('step0WorkflowEntryPosition', JS)
        self.assertIn('width * 0.05', JS)
        self.assertIn('function renderStep0StatStrip()', JS)
        self.assertIn('function step0FilteredStageStats(rows = step0FilteredSortedRows())', JS)
        self.assertIn('function applyStep0SummaryFilterTone()', JS)
        self.assertIn('function step0WorkflowNodeColor(row, stageKey, defaultColor)', JS)
        self.assertIn('function rememberStep0FilterSelection(key, { colorAllCategories = false } = {})', JS)
        self.assertIn('step0FilterSelectionOrder', JS)
        self.assertIn("colorAllCategories: value === 'all'", JS)
        self.assertIn("value === 'all' && key === 'progress'", JS)
        self.assertIn('const orderedActive = [...(state.step0FilterSelectionOrder || [])]', JS)
        self.assertIn('STEP0_WORKFLOW_FILTER_COLOR_VARIABLES', JS)
        self.assertIn('function step0WorkflowThemeColor(variable, fallback)', JS)
        self.assertIn("fill: step0WorkflowNodeColor(row, stage.key, stageColor)", workflow_map)
        self.assertIn("stroke: 'transparent'", workflow_map)
        self.assertIn('lineWidth: 0', workflow_map)
        self.assertIn('function step0WorkflowNodeSize(stageKey, nodeCount)', JS)
        self.assertIn('STEP0_WORKFLOW_NODE_SIZE_RANGES', JS)
        self.assertIn('const nodeSize = step0WorkflowNodeSize(stage.key, stageRows.length)', workflow_map)
        self.assertIn("renderStep0FilteredResults();", JS)
        self.assertIn('id="step0SummaryScopeNote"', step0)
        self.assertIn('const duration = 860', JS)
        self.assertIn('startDelay = index * STEP0_WORKFLOW_STAGE_STAGGER_MS + 80', JS)
        self.assertIn('function animateDashboardDonut(container, delay = 0)', JS)
        self.assertIn('data-donut-legend-value', JS)
        self.assertIn('cancelDashboardDonutAnimations();', JS)
        self.assertIn('function animateWorkflowBars(container, delay = 0)', JS)
        self.assertIn('data-bar-count-value', JS)
        self.assertIn('animateWorkflowBars(elements.workflowPriorityList, 220)', JS)
        self.assertIn('entryDelay', workflow_map)
        self.assertIn('entryDelay: Math.floor(step0WorkflowSeededUnit(`${id}:stagger`) * 721)', workflow_map)
        self.assertIn('arcX', workflow_map)
        self.assertIn('arcY', workflow_map)
        self.assertIn('const maxSettleDuration = 5000', JS)
        self.assertIn('const maxEntryDelay = 720', JS)
        self.assertIn('const entryReveal =', JS)
        self.assertIn('settleDuration: 1500 + Math.floor(step0WorkflowSeededUnit(`${id}:settle-duration`) * 3501)', workflow_map)
        self.assertIn('const settleFade =', JS)
        self.assertIn('settleOvershoot', workflow_map)
        self.assertIn('settlePhase', workflow_map)
        self.assertIn('animateStep0WorkflowDots', JS)
        self.assertIn('targetX: position.x', workflow_map)
        self.assertIn('targetY: position.y', workflow_map)
        self.assertIn('x: entryPosition.x', workflow_map)
        self.assertIn('y: entryPosition.y', workflow_map)
        self.assertNotIn("type: 'grid'", workflow_map)
        self.assertNotIn("type: 'd3-force'", workflow_map)
        self.assertIn('animateStep0WorkflowDots(graph, nodes, stage.index)', workflow_map)
        self.assertIn("behaviors: ['drag-element']", workflow_map)
        self.assertNotIn("'drag-canvas'", workflow_map)
        ...
        self.assertIn('.step0-summary-cards', CSS)
        self.assertIn('.step0-summary-dashboard > .step0-summary-dashboard-heading {', CSS)
        self.assertIn('.step0-summary-dashboard > .step0-summary-cards {', CSS)
        self.assertIn('otherwise receives `order: 3`', CSS)
        self.assertIn('.step0-summary-dashboard-heading[aria-expanded="false"]', CSS)
        self.assertNotIn('.step0-summary-dashboard[data-active-stage]', CSS)
        self.assertIn('.step0-summary-dashboard button.step0-stat:hover', CSS)
        self.assertIn('box-shadow: none;', CSS)
        self.assertIn('background: color-mix(in srgb, var(--accent) 8%, var(--surface));', CSS)
        self.assertIn('.step0-summary-dashboard .step0-stat-panel {', CSS)
        self.assertIn('border-radius: 10px 10px 0 0;', CSS)
        self.assertIn('step0-workflow-stage-enter', CSS)
    ...
    def test_team_review_score_history_can_store_reasons_and_summarizes_ai_qualitative_entries(self):
        ...
        self.assertIn("data-edit-history-reason-edit", history)
        self.assertIn("existingReason ? ' hidden' : ''", history)
        ...
        self.assertIn("AI 답변 삭제", history)
        self.assertIn("detail-edit-history-reason-summary", history)
    ...
    def test_workflow_description_is_a_connected_information_panel(self):
        ...
        self.assertIn('escapeHtml(description)', render)
        self.assertIn('copy.descriptionCustomProject', render)
    ...
    def test_shortlisting_decision_guide_uses_three_step_shared_structure(self):
        ...
        for formula in (
            ...
            "In-vivo = O",
            "In-vitro = O",
            "ADMET uploaded + scored (완료 Study / 25)",
    ...
    def test_priority_pipeline_shows_top_ten_with_scroll_and_breaks_ties_by_latest_search(self):
        ...
        self.assertIn("Date.parse(a.action_date || '')", render)
        ...
        self.assertIn("Action date 설정 항목 · 임박 순", JS)
    ...
    def test_tab_one_and_two_identity_fields_share_double_click_editing_and_target_wrapping(self):
        ...
        self.assertIn("Boolean(getCurrentUser())", JS)
    ...
    def test_detail_left_rail_shares_space_evenly(self):
        ...
        self.assertIn("detail.js?v=20260910-oic-criteria-pills-2", DETAIL_HTML)
    ...
    def test_review_workspace_uses_compact_version_refresh_pills_and_owner_meta(self):
        ...
        self.assertIn("${isHuman ? 'data-manual-score=\"true\"' : ''}", collaboration)
        self.assertIn("const manualScoreStyle = isHuman", collaboration)
        self.assertIn("const manualScoreBorder = {", collaboration)
        self.assertIn("var(--fluent-green)", collaboration)
        self.assertNotIn("background: color-mix(in srgb, #b9616b", collaboration)
        self.assertNotIn("color: #b9616b", collaboration)
        self.assertIn(".filter-score-panel .score-chip.is-human", CSS)
        self.assertIn("var(--fluent-green) 72%", CSS)
        ...
        self.assertIn("styles.css?v=20260910-oic-criteria-pills-1", DETAIL_HTML)
        self.assertIn("detail.js?v=20260910-oic-criteria-pills-2", DETAIL_HTML)
    ...
    def test_partner_material_body_scrolls_below_fixed_header(self):
        ...
        self.assertIn('./src/styles.css?v=20260910-oic-criteria-pills-1', DETAIL_HTML)
    ...
    def test_data_upload_reviews_exact_and_similar_assets_before_replacing_existing_record(self):
        ...
        preview = function_body(JS, "previewPastedReportParsing")
        save = function_body(JS, "saveStructuredJsonInput")
        self.assertIn("pipelineCompaniesEquivalent", finder)
        ...
        self.assertIn("companyMatch", finder)
        self.assertIn("reviewDataReuploadMatches", preview)
        self.assertIn("automaticExactReuploadDecision", preview)
        automatic_reupload = function_body(JS, "automaticExactReuploadDecision")
        self.assertIn("hasConfirmedCompanyAliasOverlap", automatic_reupload)
        self.assertIn("companyAliases", automatic_reupload)
        self.assertIn("동일 Pipeline 자동 갱신", preview)
        self.assertIn("confirmed_replacements", save)
        self.assertIn("skippedIncomingIndexes", save)
        self.assertIn("incoming_record_id", save)
        self.assertIn("existing_record_id", save)
        self.assertIn('id="dataReuploadList"', HTML)
        self.assertIn('id="dataReuploadSaveAliasesAll"', HTML)
        self.assertIn('id="dataReuploadOverwriteAll"', HTML)
        self.assertIn("검토 없이 모두 신규 업로드", HTML)
        self.assertIn("업로드 없이 유사 이름만 저장", HTML)
        self.assertIn("선택 적용", HTML)
        self.assertIn("data-reupload-comparison-scroll", JS)
        self.assertIn("pipelineCompanyMatchKind", JS)
        self.assertIn("AUTO_REUPLOAD_PLACEHOLDER_IDENTITIES", JS)
