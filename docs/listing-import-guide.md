...
`Others`라는 문자열 자체가 화면에 그대로 노출되는 곳은 **Summary Dashboard의 집계 차트(예: "SKBP 관심 적응증 분포")뿐**입니다. 이 차트는 6대 우선 적응증만 개별 항목으로 쪼개고, 나머지 전부(Library 미등록 원문 + 진짜 `Unknown`)를 하나의 `Others` 조각으로 합산해서 보여주는 요약용 라벨입니다(`src/app.js`의 `topCountsWithOthers`/관련 차트 집계 로직). Pipeline Table의 Main indication 셀, Tab 0 진척현황 표, 상세 페이지, `skbp_pipeline_wiki`/Obsidian 마크다운 export에는 `Others`라는 문자열이 나타나지 않습니다 — 이 화면들은 항상 (a) Canonical 매칭에 성공하면 Canonical 대표값을, (b) 매칭에 실패하면 원문 그대로를, (c) 원문 자체가 없으면 `Unknown`을 보여줍니다. Canonical 대표값이 표시된 셀도 마우스를 올리면 항상 원문이 함께 노출됩니다(`indicationFullHoverTitle`/`sourceFieldHoverTitle`).

정리하면 세 값의 의미가 다릅니다: **Canonical 대표값**(Library 매칭 성공, hover 시 원문 확인 가능) / **원문 그대로 표시**(Library 미등록·`Others`로만 내부 분류, 화면에는 원문 노출) / **`Unknown`**(원문 자체가 없거나 판단 근거가 없음). Canonical Library 승격은 §"Canonical Library는 반복 비교·집계할 상위 치료 플랫폼만 포함합니다" 문단의 원칙(동일 원문이 서로 다른 3개 이상 Pipeline에서 확인되거나 운영상 별도 집계가 필요할 때만 검토)을 indication에도 동일하게 적용합니다. 2026-09-09 리뷰에서 Step 0 진척현황의 미등록 원문 176종(633건 노출)을 전수 검토해 `Vascular dementia`, `Progressive supranuclear palsy / atypical parkinsonism`, `Charcot-Marie-Tooth disease / hereditary neuropathy`, `Ataxia (spinocerebellar / Friedreich)` 4개 신규 Canonical과 `Alzheimer's disease`(Mild cognitive impairment)·`Parkinson's disease`(Morning akinesia)·`Multiple sclerosis / neuroinflammatory disease`(Neuromyelitis optica/NMO)·`Stroke`(Subarachnoid/Brain hemorrhage) synonym 8종을 추가했다(`config/category-synonyms.json` v1.5). "Other neurological indications"(40건), "Neurodegenerative Diseases"(14건), "Unspecified Central Nervous System Disorders"(10건) 등 그 자체로 특정 질환을 지칭하지 않는 상위 placeholder 문구는 의도적으로 Canonical화하지 않았다 — 이런 값은 일괄 매핑 대상이 아니라 자산별 원문을 다시 확인해야 하는 개별 검토 대상이다. `Alcohol Dependence`, `Dystonia`, `Anosmia`, `Traumatic Brain Injury`처럼 1건만 확인된 값도 3건 임계값 미달로 이번에는 보류했다.
